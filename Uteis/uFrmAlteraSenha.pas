...
uses
  ...
  Dialogs, StdCtrls, DB, DBAccess, Ora, MemDS, Buttons,strutils,
  ExtCtrls, OraCall;
...
type
  TFrmAlteraSenha = class(TForm)
    ...
    Qry_BasesDATABASE_LINK: TStringField;
    Panel1: TPanel;
  ...
  public
    Filial : Integer;
    Usuario , Pconectastring , Servidor , PIPLocal: String;
...
procedure TFrmAlteraSenha.Confirma_Senha(Sender: TObject);
var
...
begin
  ...
  begin
      ...
      if ( PosEx('10.0.1.',PIPLocal,0 )>0 ) then
      begin
            if ( PosEx('192.168.0.250', Pconectastring ) > 0 ) then
               Pconectastring :=  StringReplace(Pconectastring ,'192.168.0.250','189.0.0.202',[]);
      end;
    ...
    begin
      ...
      begin
        ...
        Qry_Bases.MacroByName('Datalink').Value := '' ;
...
procedure TFrmAlteraSenha.FecharTrocaSenha(Sender: TObject);
begin
  //Troca a senha
  Close;
end;

procedure TFrmAlteraSenha.Pressionou_enter_senhas(Sender: TObject;
  var Key: Char);
begin
