object FrmProgRacao: TFrmProgRacao
  Left = 291
  Top = 63
  Width = 939
  Height = 537
  ...
  object Qr: TSmartQuery
    ...
    SQL.Strings = (
      ...
      '            THEN INIC_SEM + NVL(TRUNC(RAC_SOBRA / RP_DIA),0)'
      
        '            ELSE INIC_SEM_SEG + TRUNC(RAC_SOBRA2 / RP_DIA_SEMSEG' +
        ' )  END) PREV,'
      '            CA.* ,'
      '       TRUNC(RAC_SOBRA / RP_DIA) DIAS_RAC ,'
      '       TRUNC(RAC_SOBRA2 / RP_DIA_SEMSEG ) DIAS_RAC2'
      '  FROM ('
      'SELECT G.*,'
      '       G.AVES * RP.VAL_CONSUMO RP_SEM,'
      '       TRUNC((G.AVES * RP.VAL_CONSUMO) / 7) RP_DIA,'
      '       TRUNC((G.AVES * RS.VAL_CONSUMO) / 7) RP_DIA_SEMSEG,'
      '      ( SELECT nvl(SUM(VAL_CONSUMO* G.AVES ),0) CONS_ACUM'
      '          FROM CONSUMO_RACAO_SEMANA'
      '          WHERE SEMANA < G.SEMANA  )  RP_ACUM_ANT,'
      '      ( SELECT SUM(VAL_CONSUMO * G.AVES ) CONS_ACUM'
      '          FROM CONSUMO_RACAO_SEMANA'
      '          WHERE SEMANA <= G.SEMANA  )  RP_ACUM,'
      
        '      RACAO - ( SELECT nvl(SUM(VAL_CONSUMO* G.AVES ),0) CONS_ACU' +
        'M'
      '          FROM CONSUMO_RACAO_SEMANA'
      '          WHERE SEMANA < G.SEMANA  )    RAC_SOBRA,'
      
        '      RACAO -        ( SELECT SUM(VAL_CONSUMO * G.AVES ) CONS_AC' +
        'UM'
      '          FROM CONSUMO_RACAO_SEMANA'
      '          WHERE SEMANA <= G.SEMANA  )    RAC_SOBRA2 ,'
      '      G.DATA_ALOJAMENTO + ( ((G.Semana-1) * 7) ) INIC_SEM,'
      '      G.DATA_ALOJAMENTO + ( ((G.Semana) * 7) ) INIC_SEM_SEG'
      'FROM ('
      'Select A.ID_LOTE ,'
      '       TRUNC (A.DATA_ALOJAMENTO) DATA_ALOJAMENTO ,'
      
        '       TRUNC ( TO_DATE(  :DataTeste,'#39'DD/MM/YYYY'#39' ) - TRUNC (A.DA' +
        'TA_ALOJAMENTO)) IDADE,'
      
        '       CEIL( TRUNC( TO_DATE( :DataTeste,'#39'DD/MM/YYYY'#39' ) - TRUNC (' +
        'A.DATA_ALOJAMENTO) )  / 7 )  +'
      
        '       CASE  WHEN MOD( ( TO_DATE( :DataTeste,'#39'DD/MM/YYYY'#39' ) - TR' +
        'UNC (A.DATA_ALOJAMENTO) ) , 7) <= 0 THEN 1 ELSE 0 END'
      '       SEMANA,'
      '        A.QTD_ALOJ AVES , -- SUM(F.Quantidade) AVES,'
      '       RE.RACAO, A.PROX_ENTREGA_RACAO'
      '  From Integrado_Alojamento A'
      ' --Inner Join Integrado_Alojamento_Fornec F'
      ' --   On A.ID_LOTE = F.ID_LOTE'
      ' LEFT Join ( Select P.ID_LOTE ID_LOTE ,  '
      
        '                    SUM( CASE WHEN P.ID_TIPOPEDIVEND = 138 THEN ' +
        'NI.QN_EMBAITEMNOTAFISC * (-1)'
      ...
      '                   AND A.FL_STATUS = '#39'AB'#39
        ...
        '                   AND N.DT_SAIDNOTAFISC <= TO_DATE( :DataTeste,' +
        #39'DD/MM/YYYY'#39' )'
      '                 GROUP BY P.ID_LOTE'
      '                 ) RE'
      '      On TO_CHAR(A.ID_LOTE) = RE.ID_LOTE'
      ' Where A.FL_STATUS = '#39'AB'#39
      '/*GROUP BY A.ID_LOTE ,'
      '       A.DATA_ALOJAMENTO , RE.RACAO, A.PROX_ENTREGA_RACAO */ ) G'
      ' LEFT Join CONSUMO_RACAO_SEMANA   RP'
      '    on  G.SEMANA  = RP.SEMANA'
      ' LEFT Join CONSUMO_RACAO_SEMANA   RS'
      '    on  G.SEMANA + 1  = RS.SEMANA  ) CA'
      '&Macro'
      'ORDER BY Prev'
      ''
      ' '
    ...
    object QrSEMANA: TFloatField
      FieldName = 'SEMANA'
    end
    object QrAVES: TFloatField
      FieldName = 'AVES'
    end
    object QrRACAO: TFloatField
      FieldName = 'RACAO'
    end
    object QrPROX_ENTREGA_RACAO: TDateTimeField
      FieldName = 'PROX_ENTREGA_RACAO'
    end
    object QrRP_SEM: TFloatField
      FieldName = 'RP_SEM'
    end
    object QrRP_DIA: TFloatField
      FieldName = 'RP_DIA'
    end
    object QrRP_DIA_SEMSEG: TFloatField
      FieldName = 'RP_DIA_SEMSEG'
    end
    object QrRP_ACUM_ANT: TFloatField
      FieldName = 'RP_ACUM_ANT'
    end
    object QrRP_ACUM: TFloatField
      FieldName = 'RP_ACUM'
    end
    object QrRAC_SOBRA: TFloatField
      FieldName = 'RAC_SOBRA'
    end
    object QrRAC_SOBRA2: TFloatField
      FieldName = 'RAC_SOBRA2'
    end
    object QrINIC_SEM: TDateTimeField
      FieldName = 'INIC_SEM'
    end
    object QrINIC_SEM_SEG: TDateTimeField
      FieldName = 'INIC_SEM_SEG'
    end
    object QrDIAS_RAC: TFloatField
      FieldName = 'DIAS_RAC'
    end
