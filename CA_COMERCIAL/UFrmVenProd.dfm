object FrmVenProd: TFrmVenProd
  Left = 403
  Top = 222
  Width = 654
  Height = 146
  VertScrollBar.Range = 41
  ActiveControl = DTini
  BorderIcons = [biSystemMenu]
  BorderStyle = bsSingle
  Caption = 'Relat'#243'rio de Resumo de Vendas por Produtos'
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = 11
  Font.Name = 'helvetica'
  Font.Pitch = fpVariable
  Font.Style = []
  Position = poMainFormCenter
  Visible = True
  OnClose = FormClose
  OnDestroy = FormDestroy
  OnShow = FormShow
  TextHeight = 10
  object ToolBar1: TToolBar
    Left = 0
    Top = 0
    Width = 638
    Height = 41
    Anchors = []
    ButtonHeight = 37
    ButtonWidth = 39
    Caption = 'ToolBar1'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = 11
    Font.Name = 'helvetica'
    Font.Pitch = fpVariable
    Font.Style = []
    ParentFont = False
    TabOrder = 0
    object Sb_Sair: TSpeedButton
      Left = 0
      Top = 0
      Width = 40
      Height = 37
      Hint = 'Sair'
      Flat = True
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF0000}
      NumGlyphs = 2
      ParentShowHint = False
      ShowHint = True
      OnClick = Sb_SairClick
    end
    object ToolButton3: TToolButton
      Left = 40
      Top = 0
      Width = 596
      Caption = 'ToolButton3'
      ImageIndex = 1
      Style = tbsSeparator
    end
  end
  object Panel1: TPanel
    Left = 0
    Top = 41
    Width = 638
    Height = 66
    Align = alClient
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = 15
    Font.Name = 'Arial'
    Font.Pitch = fpVariable
    Font.Style = []
    ParentBackground = False
    ParentFont = False
    TabOrder = 1
    object Label1: TLabel
      Left = 15
      Top = 34
      Width = 92
      Height = 15
      Caption = 'Data de Entrega:'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = 15
      Font.Name = 'Arial'
      Font.Pitch = fpVariable
      Font.Style = []
      ParentFont = False
    end
    object lbDtCorte: TLabel
      Left = 1
      Top = 51
      Width = 3
      Height = 15
      Align = alBottom
      Alignment = taCenter
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = 15
      Font.Name = 'Arial'
      Font.Pitch = fpVariable
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label2: TLabel
      Left = 265
      Top = 34
      Width = 125
      Height = 15
      Caption = 'Data Bloqueio Anterior:'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = 15
      Font.Name = 'Arial'
      Font.Pitch = fpVariable
      Font.Style = []
      ParentFont = False
    end
    object DTini: TEdit
      Left = 116
      Top = 31
      Width = 101
      Height = 23
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = 15
      Font.Name = 'Arial'
      Font.Pitch = fpVariable
      Font.Style = []
      ParentFont = False
      TabOrder = 0
    end
    object BtAtivar: TBitBtn
      Left = 545
      Top = 30
      Width = 79
      Height = 25
      Caption = '&Ativar'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = 16
      Font.Name = 'Arial'
      Font.Pitch = fpVariable
      Font.Style = []
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      ParentFont = False
      TabOrder = 1
      OnClick = BtAtivarClick
    end
    object BPESQ: TBitBtn
      Left = 218
      Top = 31
      Width = 26
      Height = 23
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = 16
      Font.Name = 'Arial'
      Font.Pitch = fpVariable
      Font.Style = []
      Glyph.Data = {
        
        33333333333333333333}
      ParentFont = False
      TabOrder = 3
      OnClick = BPESQClick
    end
    object DtBloq: TEdit
      Left = 401
      Top = 31
      Width = 101
      Height = 23
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = 15
      Font.Name = 'Arial'
      Font.Pitch = fpVariable
      Font.Style = []
      ParentFont = False
      TabOrder = 2
    end
    object BitBtn1: TBitBtn
      Left = 503
      Top = 31
      Width = 26
      Height = 23
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = 16
      Font.Name = 'Arial'
      Font.Pitch = fpVariable
      Font.Style = []
      Glyph.Data = {
        
        33333333333333333333}
      ParentFont = False
      TabOrder = 4
      OnClick = BitBtn1Click
    end
  end
  object dlgSaveArquivo: TSaveDialog
    DefaultExt = 'csv'
    FileName = 'VendaProduto.csv'
    Filter = 'Planilha sem formata'#231#227'o|*.csv'
    FilterIndex = 0
    Title = 'Save'
    Left = 56
    Top = 8
  end
  object QrTotConta: TOraQuery
    Session = FrmPrincipal.DB
    SQL.Strings = (
      'SELECT '
      '  PEDIDO_VENDA.ID_CONTA,'
      '  CONTA.NM_CONTA,'
      '  COUNT(0) as QTDE_PEDIDOS,'
      '  SUM(PEDIDO_VENDA.VL_PEDIVEND) AS TOTAL_PEDIDO,'
      '  SUM(PEDIDO_VENDA.VL_FATUPEDIVEND) AS TOTAL_FATURADO,'
      '  NVL( (SELECT '
      '          SUM(P.VL_FATUPEDIVEND)'
      '        FROM                 '
      '          PEDIDO_VENDA P,       '
      '          CONTA C,'
      '          EXPEDICAO_CARGA E,    '
      '          PEDIDO_VENDA_NOTA_FISCAL V, '
      '          NOTA_FISCAL N'
      '        WHERE          '
      '          P.ID_CONTA = C.ID_CONTA '
      '          AND (P.ID_CARGEXPE = E.ID_CARGEXPE)'
      '          AND V.ID_PEDIVEND = P.ID_PEDIVEND '
      '          AND N.ID_NOTAFISC = V.ID_NOTAFISC  '
      '          AND NVL(N.FL_CANCNOTAFISC,'#39'N'#39') = '#39'N'#39'  AND '
      '          P.ID_TIPOPEDIVEND IN (select '
      
        '                                  Tipo_oper_pedi_vend_tipo.Id_Ti' +
        'popedivend '
      '                                from '
      '                                  operacao_tipo, '
      '                                  Tipo_oper_pedi_vend_tipo '
      '                                where '
      
        '                                  operacao_tipo.id_tipooper = Ti' +
        'po_oper_pedi_vend_tipo.id_tipooper and '
      
        '                                  operacao_tipo.FL_ENTRSAIDTIPOO' +
        'PER = '#39'S'#39') '
      
        '          AND P.DT_ENTRPEDIVEND = TO_DATE('#39'11/09/2014'#39','#39'DD/MM/YY' +
        'YY'#39')'
      
        '          AND P.DT_BLOQAJUSFINAPEDIVEND > TO_DATE('#39'11/09/2014 08' +
        ':20:16'#39', '#39'DD/MM/YYYY HH24:MI:SS'#39')'
      
        '          AND P.ID_CONTA = PEDIDO_VENDA.ID_CONTA ),0) AS TOTAL_F' +
        'ATURADO_ANT'
      'FROM                 '
      '  PEDIDO_VENDA,       '
      '  CONTA,'
      '  EXPEDICAO_CARGA,    '
      '  PEDIDO_VENDA_NOTA_FISCAL, '
      '  NOTA_FISCAL'
      'WHERE          '
      '  PEDIDO_VENDA.ID_CONTA = CONTA.ID_CONTA '
      '  AND (PEDIDO_VENDA.ID_CARGEXPE = EXPEDICAO_CARGA.ID_CARGEXPE)'
      
        '  AND PEDIDO_VENDA_NOTA_FISCAL.ID_PEDIVEND = PEDIDO_VENDA.ID_PED' +
        'IVEND '
      
        '  AND NOTA_FISCAL.ID_NOTAFISC = PEDIDO_VENDA_NOTA_FISCAL.ID_NOTA' +
        'FISC  '
      '  AND NVL(NOTA_FISCAL.FL_CANCNOTAFISC,'#39'N'#39') = '#39'N'#39'  AND '
      '  PEDIDO_VENDA.ID_TIPOPEDIVEND IN (select '
      
        '                                     Tipo_oper_pedi_vend_tipo.Id' +
        '_Tipopedivend '
      '                                   from '
      '                                     operacao_tipo, '
      '                                     Tipo_oper_pedi_vend_tipo '
      '                                   where '
      
        '                                     operacao_tipo.id_tipooper =' +
        ' Tipo_oper_pedi_vend_tipo.id_tipooper and '
      
        '                                     operacao_tipo.FL_ENTRSAIDTI' +
        'POOPER = '#39'S'#39') AND'
      ''
      
        '  PEDIDO_VENDA.DT_ENTRPEDIVEND = TO_DATE('#39'12/09/2014'#39','#39'DD/MM/YYY' +
        'Y'#39')  '
      '  AND EXPEDICAO_CARGA.FL_STATCARGEXPE IN ('#39'NF'#39','#39'FE'#39')'
      '  AND PEDIDO_VENDA.FL_STATPEDIVEND <> '#39'ZR'#39' '
      'GROUP BY    '
      '  PEDIDO_VENDA.ID_CONTA, '
      '  CONTA.NM_CONTA'
      'ORDER BY  1'
      '/*'
      'SELECT '
      '  PEDIDO_VENDA.ID_CONTA,'
      '  CONTA.NM_CONTA,'
      '  COUNT(0) as QTDE_PEDIDOS,'
      '  SUM(PEDIDO_VENDA.VL_PEDIVEND) AS TOTAL_PEDIDO,'
      '  SUM(PEDIDO_VENDA.VL_FATUPEDIVEND) AS TOTAL_FATURADO'
      'FROM                 '
      '  PEDIDO_VENDA,       '
      '  CONTA,'
      '  EXPEDICAO_CARGA,    '
      '  PEDIDO_VENDA_NOTA_FISCAL, '
      '  NOTA_FISCAL'
      'WHERE          '
      '  PEDIDO_VENDA.ID_CONTA = CONTA.ID_CONTA '
      '  AND (PEDIDO_VENDA.ID_CARGEXPE = EXPEDICAO_CARGA.ID_CARGEXPE)'
      
        '  AND PEDIDO_VENDA_NOTA_FISCAL.ID_PEDIVEND = PEDIDO_VENDA.ID_PED' +
        'IVEND '
      
        '  AND NOTA_FISCAL.ID_NOTAFISC = PEDIDO_VENDA_NOTA_FISCAL.ID_NOTA' +
        'FISC  '
      '  AND NVL(NOTA_FISCAL.FL_CANCNOTAFISC,'#39'N'#39') = '#39'N'#39'  AND '
      '  PEDIDO_VENDA.ID_TIPOPEDIVEND IN (select '
      
        '                                     Tipo_oper_pedi_vend_tipo.Id' +
        '_Tipopedivend '
      '                                   from '
      '                                     operacao_tipo, '
      '                                     Tipo_oper_pedi_vend_tipo '
      '                                   where '
      
        '                                     operacao_tipo.id_tipooper =' +
        ' Tipo_oper_pedi_vend_tipo.id_tipooper and '
      
        '                                     operacao_tipo.FL_ENTRSAIDTI' +
        'POOPER = '#39'S'#39') AND'
      ''
      
        '  PEDIDO_VENDA.DT_ENTRPEDIVEND = :DT_ENTREGA --TO_DATE('#39'04/08/20' +
        '14'#39','#39'DD/MM/YYYY'#39')  '
      '  AND EXPEDICAO_CARGA.FL_STATCARGEXPE IN ('#39'NF'#39','#39'FE'#39')'
      '  AND PEDIDO_VENDA.FL_STATPEDIVEND <> '#39'ZR'#39' '
      'GROUP BY    '
      '  PEDIDO_VENDA.ID_CONTA, '
      '  CONTA.NM_CONTA'
      'ORDER BY  1'
      '*/')
    Left = 108
    Top = 17
    object QrTotContaID_CONTA: TFloatField
      FieldName = 'ID_CONTA'
      Required = True
    end
    object QrTotContaNM_CONTA: TStringField
      FieldName = 'NM_CONTA'
      Required = True
      Size = 40
    end
    object QrTotContaQTDE_PEDIDOS: TFloatField
      FieldName = 'QTDE_PEDIDOS'
    end
    object QrTotContaTOTAL_PEDIDO: TFloatField
      FieldName = 'TOTAL_PEDIDO'
    end
    object QrTotContaTOTAL_FATURADO: TFloatField
      FieldName = 'TOTAL_FATURADO'
    end
    object QrTotContaTOTAL_FATURADO_ANT: TFloatField
      FieldName = 'TOTAL_FATURADO_ANT'
    end
  end
  object Qr: TOraQuery
    Session = FrmPrincipal.DB
    SQL.Strings = (
      '-- ANTES DO CORTE'
      'SELECT '
      '  '#39'Antes do Corte'#39' AS CORTE,'
      '  MATERIAL_EMBALAGEM.ID_PRODMATEEMBA, '
      '  MATERIAL_EMBALAGEM.NM_PRODMATEEMBA, '
      '  Sum(Nvl(PEDIDO_VENDA_ITEM.qn_embaitempedivend,0)) as Cx_Ped, '
      '  Sum(Nvl(PEDIDO_VENDA_ITEM.qn_pesoitempedivend,0)) as Kg_Ped, '
      
        '  SUM(PEDIDO_VENDA_ITEM.qn_pesoitempedivend*PEDIDO_VENDA_ITEM.VL' +
        '_UNITITEMPEDIVEND)/(CASE WHEN Sum(Nvl(PEDIDO_VENDA_ITEM.qn_pesoi' +
        'tempedivend,0))=0 THEN 1 ELSE Sum(Nvl(PEDIDO_VENDA_ITEM.qn_pesoi' +
        'tempedivend,0)) END) AS Vl_Unit, '
      
        '  SUM(PEDIDO_VENDA_ITEM.qn_pesoitempedivend*PEDIDO_VENDA_ITEM.VL' +
        '_UNITITEMPEDIVEND) AS Vl_Total, '
      
        '  Sum(Nvl(PEDIDO_VENDA_ITEM.qn_embaexpeitempedivend,0)) as Cx_Ex' +
        'ped,     '
      
        '  Sum(Nvl(PEDIDO_VENDA_ITEM.QN_PESOPADREXPEITEMPEDIVEND,0)) as K' +
        'g_Exped, '
      
        '  (Sum(Nvl(PEDIDO_VENDA_ITEM.qn_pesoitempedivend,0)) - Sum(Nvl(P' +
        'EDIDO_VENDA_ITEM.QN_PESOPADREXPEITEMPEDIVEND,0)) ) as Kg_Dif,'
      
        '  Sum(PEDIDO_VENDA_ITEM.Qn_Pesopadrexpeitempedivend*PEDIDO_VENDA' +
        '_ITEM.VL_UNITITEMPEDIVEND) AS Vl_Faturado '
      'FROM                 '
      '  PEDIDO_VENDA_ITEM,  '
      '  PEDIDO_VENDA,       '
      '  EXPEDICAO_CARGA,    '
      '  CLIENTE,            '
      '  VENDEDOR,           '
      '  MATERIAL_EMBALAGEM, '
      '  MATERIAL,           '
      '  PEDIDO_VENDA_NOTA_FISCAL, '
      '  NOTA_FISCAL,  '
      '  BAIRRO,       '
      '  CIDADE,       '
      '  ESTADO        '
      'WHERE          '
      
        '  (PEDIDO_VENDA_ITEM.ID_PEDIVEND = PEDIDO_VENDA.ID_PEDIVEND) AND' +
        '       '
      
        '  (PEDIDO_VENDA_ITEM.ID_MATEEMBA = MATERIAL_EMBALAGEM.ID_MATEEMB' +
        'A) AND '
      
        '  (PEDIDO_VENDA.ID_CARGEXPE = EXPEDICAO_CARGA.ID_CARGEXPE) AND  ' +
        '    '
      '  (PEDIDO_VENDA.ID_CLIENTE = CLIENTE.ID_CLIENTE) AND          '
      '  (PEDIDO_VENDA.ID_VENDEDOR = VENDEDOR.ID_VENDEDOR(+)) AND    '
      '  MATERIAL.ID_MATERIAL = MATERIAL_EMBALAGEM.ID_MATERIAL AND   '
      
        '  PEDIDO_VENDA_NOTA_FISCAL.ID_PEDIVEND = PEDIDO_VENDA.ID_PEDIVEN' +
        'D AND  '
      
        '  NOTA_FISCAL.ID_NOTAFISC = PEDIDO_VENDA_NOTA_FISCAL.ID_NOTAFISC' +
        '  AND  '
      '  NVL(NOTA_FISCAL.FL_CANCNOTAFISC,'#39'N'#39') = '#39'N'#39'  AND '
      '  BAIRRO.ID_BAIRRO = CLIENTE.ID_BAIRRO AND      '
      '  CIDADE.ID_CIDADE = BAIRRO.ID_CIDADE AND       '
      '  ESTADO.ID_ESTADO = CIDADE.ID_ESTADO AND       '
      
        '  PEDIDO_VENDA.DT_ENTRPEDIVEND = :DT_ENTREGA --TO_DATE('#39'02/08/20' +
        '14'#39','#39'DD/MM/YYYY'#39') '
      
        '  AND PEDIDO_VENDA.DT_DIGIPEDIVEND <= :DT_CORTE --(select max(dt' +
        'hora_corte) from pedido_venda where dt_entrpedivend = :DT_ENTREG' +
        'A)     '
      
        '  AND PEDIDO_VENDA.ID_TIPOPEDIVEND IN (select Tipo_oper_pedi_ven' +
        'd_tipo.Id_Tipopedivend '
      
        '                                        from operacao_tipo, tipo' +
        '_oper_pedi_vend_tipo '
      
        '                                          where operacao_tipo.id' +
        '_tipooper = Tipo_oper_pedi_vend_tipo.id_tipooper and '
      
        '                                           operacao_tipo.FL_ENTR' +
        'SAIDTIPOOPER = '#39'S'#39' )  '
      'GROUP BY  '
      '  '#39'A'#39','
      '  MATERIAL_EMBALAGEM.ID_PRODMATEEMBA,  '
      '  MATERIAL_EMBALAGEM.NM_PRODMATEEMBA   '
      'UNION ALL   '
      ''
      '-- DEPOIS DO CORTE'
      'SELECT '
      '  '#39'Depois do Corte'#39' AS CORTE,'
      '  MATERIAL_EMBALAGEM.ID_PRODMATEEMBA, '
      '  MATERIAL_EMBALAGEM.NM_PRODMATEEMBA, '
      '  Sum(Nvl(PEDIDO_VENDA_ITEM.qn_embaitempedivend,0)) as Cx_Ped, '
      '  Sum(Nvl(PEDIDO_VENDA_ITEM.qn_pesoitempedivend,0)) as Kg_Ped, '
      
        '  SUM(PEDIDO_VENDA_ITEM.qn_pesoitempedivend*PEDIDO_VENDA_ITEM.VL' +
        '_UNITITEMPEDIVEND)/(CASE WHEN Sum(Nvl(PEDIDO_VENDA_ITEM.qn_pesoi' +
        'tempedivend,0))=0 THEN 1 ELSE Sum(Nvl(PEDIDO_VENDA_ITEM.qn_pesoi' +
        'tempedivend,0)) END) AS Vl_Unit, '
      
        '  SUM(PEDIDO_VENDA_ITEM.qn_pesoitempedivend*PEDIDO_VENDA_ITEM.VL' +
        '_UNITITEMPEDIVEND) AS Vl_Total, '
      
        '  Sum(Nvl(PEDIDO_VENDA_ITEM.qn_embaexpeitempedivend,0)) as Cx_Ex' +
        'ped,     '
      
        '  Sum(Nvl(PEDIDO_VENDA_ITEM.QN_PESOPADREXPEITEMPEDIVEND,0)) as K' +
        'g_Exped, '
      
        '  (Sum(Nvl(PEDIDO_VENDA_ITEM.qn_pesoitempedivend,0)) - Sum(Nvl(P' +
        'EDIDO_VENDA_ITEM.QN_PESOPADREXPEITEMPEDIVEND,0)) ) as Kg_Dif,'
      
        '  Sum(PEDIDO_VENDA_ITEM.Qn_Pesopadrexpeitempedivend*PEDIDO_VENDA' +
        '_ITEM.VL_UNITITEMPEDIVEND) AS Vl_Faturado '
      'FROM                 '
      '  PEDIDO_VENDA_ITEM,  '
      '  PEDIDO_VENDA,       '
      '  EXPEDICAO_CARGA,    '
      '  CLIENTE,            '
      '  VENDEDOR,           '
      '  MATERIAL_EMBALAGEM, '
      '  MATERIAL,           '
      '  PEDIDO_VENDA_NOTA_FISCAL, '
      '  NOTA_FISCAL,  '
      '  BAIRRO,       '
      '  CIDADE,       '
      '  ESTADO        '
      'WHERE          '
      
        '  (PEDIDO_VENDA_ITEM.ID_PEDIVEND = PEDIDO_VENDA.ID_PEDIVEND) AND' +
        '       '
      
        '  (PEDIDO_VENDA_ITEM.ID_MATEEMBA = MATERIAL_EMBALAGEM.ID_MATEEMB' +
        'A) AND '
      
        '  (PEDIDO_VENDA.ID_CARGEXPE = EXPEDICAO_CARGA.ID_CARGEXPE) AND  ' +
        '    '
      '  (PEDIDO_VENDA.ID_CLIENTE = CLIENTE.ID_CLIENTE) AND          '
      '  (PEDIDO_VENDA.ID_VENDEDOR = VENDEDOR.ID_VENDEDOR(+)) AND    '
      '  MATERIAL.ID_MATERIAL = MATERIAL_EMBALAGEM.ID_MATERIAL AND   '
      
        '  PEDIDO_VENDA_NOTA_FISCAL.ID_PEDIVEND = PEDIDO_VENDA.ID_PEDIVEN' +
        'D AND  '
      
        '  NOTA_FISCAL.ID_NOTAFISC = PEDIDO_VENDA_NOTA_FISCAL.ID_NOTAFISC' +
        '  AND  '
      '  NVL(NOTA_FISCAL.FL_CANCNOTAFISC,'#39'N'#39') = '#39'N'#39'  AND '
      '  BAIRRO.ID_BAIRRO = CLIENTE.ID_BAIRRO AND      '
      '  CIDADE.ID_CIDADE = BAIRRO.ID_CIDADE AND       '
      '  ESTADO.ID_ESTADO = CIDADE.ID_ESTADO AND       '
      
        '  PEDIDO_VENDA.DT_ENTRPEDIVEND = :DT_ENTREGA AND --TO_DATE('#39'02/0' +
        '8/2014'#39','#39'DD/MM/YYYY'#39') AND '
      
        '  PEDIDO_VENDA.DT_DIGIPEDIVEND > :DT_CORTE --(select max(dthora_' +
        'corte) from pedido_venda where dt_entrpedivend = :DT_ENTREGA)'
      
        '  AND PEDIDO_VENDA.ID_TIPOPEDIVEND IN (select Tipo_oper_pedi_ven' +
        'd_tipo.Id_Tipopedivend '
      
        '                                    from operacao_tipo, Tipo_ope' +
        'r_pedi_vend_tipo '
      
        '                                     where operacao_tipo.id_tipo' +
        'oper = Tipo_oper_pedi_vend_tipo.id_tipooper and '
      
        '                                       operacao_tipo.FL_ENTRSAID' +
        'TIPOOPER = '#39'S'#39' )  '
      'GROUP BY '
      '  MATERIAL_EMBALAGEM.ID_PRODMATEEMBA,  '
      '  MATERIAL_EMBALAGEM.NM_PRODMATEEMBA   '
      'ORDER BY 1,2'
      ' ')
    Left = 399
    Top = 20
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'DT_ENTREGA'
        Value = nil
      end
      item
        DataType = ftUnknown
        Name = 'DT_CORTE'
        Value = nil
      end>
    object QrCORTE: TStringField
      FieldName = 'CORTE'
      Size = 15
    end
    object QrID_PRODMATEEMBA: TStringField
      FieldName = 'ID_PRODMATEEMBA'
      Size = 5
    end
    object QrNM_PRODMATEEMBA: TStringField
      FieldName = 'NM_PRODMATEEMBA'
      Size = 40
    end
    object QrCX_PED: TFloatField
      FieldName = 'CX_PED'
      DisplayFormat = '#,##0'
    end
    object QrKG_PED: TFloatField
      FieldName = 'KG_PED'
      DisplayFormat = '#,##0.00'
    end
    object QrVL_UNIT: TFloatField
      FieldName = 'VL_UNIT'
      DisplayFormat = '#,##0.0000'
    end
    object QrVL_TOTAL: TFloatField
      FieldName = 'VL_TOTAL'
      DisplayFormat = '#,##0.00'
    end
    object QrCX_EXPED: TFloatField
      FieldName = 'CX_EXPED'
      DisplayFormat = '#,##0'
    end
    object QrKG_EXPED: TFloatField
      FieldName = 'KG_EXPED'
      DisplayFormat = '#,##0.00'
    end
    object QrKG_DIF: TFloatField
      FieldName = 'KG_DIF'
      DisplayFormat = '#,##0.00'
    end
    object QrVL_FATURADO: TFloatField
      FieldName = 'VL_FATURADO'
      DisplayFormat = '#,##0.00'
    end
  end
  object DS: TDataSource
    DataSet = Qr
    Left = 36
    Top = 131
  end
  object QrTotTipPedVend: TOraQuery
    Session = FrmPrincipal.DB
    SQL.Strings = (
      'SELECT '
      '  PEDIDO_VENDA.ID_TIPOPEDIVEND,'
      '  PEDIDO_VENDA_TIPO.NM_TIPOPEDIVEND,'
      '  PEDIDO_VENDA.ID_CONTA,'
      '  CONTA.NM_CONTA,'
      '  COUNT(0) as QTDE_PEDIDOS,'
      '  NVL(SUM(PEDIDO_VENDA.VL_PEDIVEND),0) AS TOTAL_PEDIDO,'
      '  NVL(SUM(PEDIDO_VENDA.VL_FATUPEDIVEND),0) AS TOTAL_FATURADO'
      'FROM                 '
      '  PEDIDO_VENDA,       '
      '  PEDIDO_VENDA_TIPO,'
      '  EXPEDICAO_CARGA,    '
      '  PEDIDO_VENDA_NOTA_FISCAL, '
      '  NOTA_FISCAL,'
      '  CONTA'
      'WHERE          '
      
        '  PEDIDO_VENDA.ID_TIPOPEDIVEND = PEDIDO_VENDA_TIPO.ID_TIPOPEDIVE' +
        'ND '
      '  AND (PEDIDO_VENDA.ID_CARGEXPE = EXPEDICAO_CARGA.ID_CARGEXPE)'
      
        '  AND PEDIDO_VENDA_NOTA_FISCAL.ID_PEDIVEND = PEDIDO_VENDA.ID_PED' +
        'IVEND '
      
        '  AND NOTA_FISCAL.ID_NOTAFISC = PEDIDO_VENDA_NOTA_FISCAL.ID_NOTA' +
        'FISC  '
      '  AND PEDIDO_VENDA.ID_CONTA = CONTA.ID_CONTA'
      '  AND NVL(NOTA_FISCAL.FL_CANCNOTAFISC,'#39'N'#39') = '#39'N'#39'  AND '
      '  PEDIDO_VENDA.ID_TIPOPEDIVEND IN (select '
      
        '                                     Tipo_oper_pedi_vend_tipo.Id' +
        '_Tipopedivend '
      '                                   from '
      '                                     operacao_tipo, '
      '                                     Tipo_oper_pedi_vend_tipo '
      '                                   where '
      
        '                                     operacao_tipo.id_tipooper =' +
        ' Tipo_oper_pedi_vend_tipo.id_tipooper and '
      
        '                                     operacao_tipo.FL_ENTRSAIDTI' +
        'POOPER = '#39'S'#39') AND'
      ''
      
        '  PEDIDO_VENDA.DT_ENTRPEDIVEND = :DT_ENTREGA --TO_DATE('#39'11/08/20' +
        '14'#39','#39'DD/MM/YYYY'#39')  '
      '  AND EXPEDICAO_CARGA.FL_STATCARGEXPE IN ('#39'NF'#39','#39'FE'#39')'
      '  AND PEDIDO_VENDA.FL_STATPEDIVEND <> '#39'ZR'#39' '
      'GROUP BY  '
      '  PEDIDO_VENDA.ID_TIPOPEDIVEND, '
      '  PEDIDO_VENDA_TIPO.NM_TIPOPEDIVEND,'
      '  PEDIDO_VENDA.ID_CONTA,'
      '  CONTA.NM_CONTA'
      'ORDER BY 2,3'
      ''
      '/*'
      'SELECT '
      '  PEDIDO_VENDA.ID_TIPOPEDIVEND,'
      '  PEDIDO_VENDA_TIPO.NM_TIPOPEDIVEND,'
      '  COUNT(0) as QTDE_PEDIDOS,'
      '  NVL(SUM(PEDIDO_VENDA.VL_PEDIVEND),0) AS TOTAL_PEDIDO,'
      '  NVL(SUM(PEDIDO_VENDA.VL_FATUPEDIVEND),0) AS TOTAL_FATURADO'
      'FROM                 '
      '  PEDIDO_VENDA,       '
      '  PEDIDO_VENDA_TIPO,'
      '  EXPEDICAO_CARGA,    '
      '  PEDIDO_VENDA_NOTA_FISCAL, '
      '  NOTA_FISCAL'
      'WHERE          '
      
        '  PEDIDO_VENDA.ID_TIPOPEDIVEND = PEDIDO_VENDA_TIPO.ID_TIPOPEDIVE' +
        'ND '
      '  AND (PEDIDO_VENDA.ID_CARGEXPE = EXPEDICAO_CARGA.ID_CARGEXPE)'
      
        '  AND PEDIDO_VENDA_NOTA_FISCAL.ID_PEDIVEND = PEDIDO_VENDA.ID_PED' +
        'IVEND '
      
        '  AND NOTA_FISCAL.ID_NOTAFISC = PEDIDO_VENDA_NOTA_FISCAL.ID_NOTA' +
        'FISC  '
      '  AND NVL(NOTA_FISCAL.FL_CANCNOTAFISC,'#39'N'#39') = '#39'N'#39'  AND '
      '  PEDIDO_VENDA.ID_TIPOPEDIVEND IN (select '
      
        '                                     Tipo_oper_pedi_vend_tipo.Id' +
        '_Tipopedivend '
      '                                   from '
      '                                     operacao_tipo, '
      '                                     Tipo_oper_pedi_vend_tipo '
      '                                   where '
      
        '                                     operacao_tipo.id_tipooper =' +
        ' Tipo_oper_pedi_vend_tipo.id_tipooper and '
      
        '                                     operacao_tipo.FL_ENTRSAIDTI' +
        'POOPER = '#39'S'#39') AND'
      ''
      
        '  PEDIDO_VENDA.DT_ENTRPEDIVEND = :DT_ENTREGA --TO_DATE('#39'04/08/20' +
        '14'#39','#39'DD/MM/YYYY'#39')  '
      '     '
      'GROUP BY  '
      '  PEDIDO_VENDA.ID_TIPOPEDIVEND, '
      '  PEDIDO_VENDA_TIPO.NM_TIPOPEDIVEND'
      'ORDER BY 1'
      '*/')
    Left = 201
    Top = 9
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'DT_ENTREGA'
        Value = nil
      end>
    object QrTotTipPedVendID_TIPOPEDIVEND: TFloatField
      FieldName = 'ID_TIPOPEDIVEND'
      Required = True
    end
    object QrTotTipPedVendNM_TIPOPEDIVEND: TStringField
      FieldName = 'NM_TIPOPEDIVEND'
      Required = True
      Size = 40
    end
    object QrTotTipPedVendQTDE_PEDIDOS: TFloatField
      FieldName = 'QTDE_PEDIDOS'
    end
    object QrTotTipPedVendTOTAL_PEDIDO: TFloatField
      FieldName = 'TOTAL_PEDIDO'
    end
    object QrTotTipPedVendTOTAL_FATURADO: TFloatField
      FieldName = 'TOTAL_FATURADO'
    end
    object QrTotTipPedVendID_CONTA: TFloatField
      FieldName = 'ID_CONTA'
      Required = True
    end
    object QrTotTipPedVendNM_CONTA: TStringField
      FieldName = 'NM_CONTA'
      Required = True
      Size = 40
    end
  end
  object QrTotVencto: TOraQuery
    Session = FrmPrincipal.DB
    SQL.Strings = (
      'SELECT '
      '  COUNT(0) as QTDE_PEDIDOS,'
      '  CONTA.NM_CONTA,'
      '  CA_TITUREC.RDATAVENC,'
      '  SUM(CA_TITUREC.RVALODOCU) AS TOTAL'
      'FROM                 '
      '  PEDIDO_VENDA,       '
      '  PEDIDO_VENDA_NOTA_FISCAL, '
      '  NOTA_FISCAL,'
      '  CA_TITUREC,'
      '  CONTA'
      'WHERE          '
      '  PEDIDO_VENDA.ID_PEDIVEND = CA_TITUREC.RNUMEDOCU'
      
        '  AND PEDIDO_VENDA_NOTA_FISCAL.ID_PEDIVEND = PEDIDO_VENDA.ID_PED' +
        'IVEND '
      
        '  AND NOTA_FISCAL.ID_NOTAFISC = PEDIDO_VENDA_NOTA_FISCAL.ID_NOTA' +
        'FISC  '
      '  AND PEDIDO_VENDA.ID_CONTA = CONTA.ID_CONTA'
      '  AND NVL(NOTA_FISCAL.FL_CANCNOTAFISC,'#39'N'#39') = '#39'N'#39'  AND '
      '  PEDIDO_VENDA.ID_TIPOPEDIVEND IN (select '
      
        '                                     Tipo_oper_pedi_vend_tipo.Id' +
        '_Tipopedivend '
      '                                   from '
      '                                     operacao_tipo, '
      '                                     Tipo_oper_pedi_vend_tipo '
      '                                   where '
      
        '                                     operacao_tipo.id_tipooper =' +
        ' Tipo_oper_pedi_vend_tipo.id_tipooper and '
      
        '                                     operacao_tipo.FL_ENTRSAIDTI' +
        'POOPER = '#39'S'#39') AND'
      ''
      
        '  PEDIDO_VENDA.DT_ENTRPEDIVEND = :DT_ENTREGA --TO_DATE('#39'11/08/20' +
        '14'#39','#39'DD/MM/YYYY'#39')     '
      'GROUP BY'
      '  CONTA.NM_CONTA,'
      '  CA_TITUREC.RDATAVENC'
      'ORDER BY 2,3'
      ''
      '/*'
      'SELECT '
      '  COUNT(0) as QTDE_PEDIDOS,'
      '  CA_TITUREC.RDATAVENC,'
      '  SUM(CA_TITUREC.RVALODOCU) AS TOTAL'
      'FROM                 '
      '  PEDIDO_VENDA,       '
      '  PEDIDO_VENDA_NOTA_FISCAL, '
      '  NOTA_FISCAL,'
      '  CA_TITUREC'
      'WHERE          '
      '  PEDIDO_VENDA.ID_PEDIVEND = CA_TITUREC.RNUMEDOCU'
      
        '  AND PEDIDO_VENDA_NOTA_FISCAL.ID_PEDIVEND = PEDIDO_VENDA.ID_PED' +
        'IVEND '
      
        '  AND NOTA_FISCAL.ID_NOTAFISC = PEDIDO_VENDA_NOTA_FISCAL.ID_NOTA' +
        'FISC  '
      '  AND NVL(NOTA_FISCAL.FL_CANCNOTAFISC,'#39'N'#39') = '#39'N'#39'  AND '
      '  PEDIDO_VENDA.ID_TIPOPEDIVEND IN (select '
      
        '                                     Tipo_oper_pedi_vend_tipo.Id' +
        '_Tipopedivend '
      '                                   from '
      '                                     operacao_tipo, '
      '                                     Tipo_oper_pedi_vend_tipo '
      '                                   where '
      
        '                                     operacao_tipo.id_tipooper =' +
        ' Tipo_oper_pedi_vend_tipo.id_tipooper and '
      
        '                                     operacao_tipo.FL_ENTRSAIDTI' +
        'POOPER = '#39'S'#39') AND'
      ''
      
        '  PEDIDO_VENDA.DT_ENTRPEDIVEND = :DT_ENTREGA --TO_DATE('#39'04/08/20' +
        '14'#39','#39'DD/MM/YYYY'#39')     '
      'GROUP BY'
      '  CA_TITUREC.RDATAVENC'
      'ORDER BY 2'
      '*/')
    Left = 348
    Top = 21
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'DT_ENTREGA'
        Value = nil
      end>
    object QrTotVenctoQTDE_PEDIDOS: TFloatField
      FieldName = 'QTDE_PEDIDOS'
    end
    object QrTotVenctoRDATAVENC: TDateTimeField
      FieldName = 'RDATAVENC'
    end
    object QrTotVenctoTOTAL: TFloatField
      FieldName = 'TOTAL'
    end
    object QrTotVenctoNM_CONTA: TStringField
      FieldName = 'NM_CONTA'
      Required = True
      Size = 40
    end
  end
  object QrCargas: TOraQuery
    Session = FrmPrincipal.DB
    SQL.Strings = (
      'SELECT '
      '  CONTA.ID_CONTA,'
      '  CONTA.NM_CONTA,'
      '  PEDIDO_VENDA.ID_PEDIVEND,'
      '  CLIENTE.NM_CLIENTE,'
      '  EXPEDICAO_CARGA.FL_STATCARGEXPE,'
      '  PEDIDO_VENDA.DT_ENTRPEDIVEND,'
      '  NVL(PEDIDO_VENDA.QN_PESOPEDIVEND,0) AS PESO_TOTAL,'
      '  NVL(PEDIDO_VENDA.VL_PEDIVEND,0) AS TOTAL_PEDIDO'
      'FROM                 '
      '  PEDIDO_VENDA,       '
      '  EXPEDICAO_CARGA,'
      '  CLIENTE,'
      '  CONTA'
      'WHERE          '
      '  PEDIDO_VENDA.ID_CARGEXPE = EXPEDICAO_CARGA.ID_CARGEXPE'
      '  AND PEDIDO_VENDA.ID_CLIENTE = CLIENTE.ID_CLIENTE'
      '  AND PEDIDO_VENDA.ID_CONTA = CONTA.ID_CONTA'
      '  AND EXPEDICAO_CARGA.FL_STATCARGEXPE NOT IN ('#39'FE'#39','#39'NF'#39')'
      '  AND PEDIDO_VENDA.ID_TIPOPEDIVEND IN (select '
      
        '                                     Tipo_oper_pedi_vend_tipo.Id' +
        '_Tipopedivend '
      '                                   from '
      '                                     operacao_tipo, '
      '                                     Tipo_oper_pedi_vend_tipo '
      '                                   where '
      
        '                                     operacao_tipo.id_tipooper =' +
        ' Tipo_oper_pedi_vend_tipo.id_tipooper and '
      
        '                                     operacao_tipo.FL_ENTRSAIDTI' +
        'POOPER = '#39'S'#39')'
      
        '  AND PEDIDO_VENDA.DT_ENTRPEDIVEND <= :DT_ENTREGA --TO_DATE('#39'22/' +
        '08/2014'#39','#39'DD/MM/YYYY'#39')  '
      ''
      'UNION ALL'
      '  '
      '--pedidos sem carga'
      'SELECT '
      '  CONTA.ID_CONTA,'
      '  CONTA.NM_CONTA,'
      '  PEDIDO_VENDA.ID_PEDIVEND,'
      '  CLIENTE.NM_CLIENTE,'
      '  '#39'S/C'#39' AS FL_STATCARGEXPE,'
      '  PEDIDO_VENDA.DT_ENTRPEDIVEND,'
      '  NVL(PEDIDO_VENDA.QN_PESOPEDIVEND,0) AS PESO_TOTAL,'
      '  NVL(PEDIDO_VENDA.VL_PEDIVEND,0) AS TOTAL_PEDIDO'
      'FROM                 '
      '  PEDIDO_VENDA,       '
      '  CLIENTE,'
      '  CONTA'
      'WHERE          '
      '  PEDIDO_VENDA.ID_CLIENTE = CLIENTE.ID_CLIENTE'
      '  AND PEDIDO_VENDA.ID_CONTA = CONTA.ID_CONTA'
      '  AND PEDIDO_VENDA.ID_TIPOPEDIVEND IN (select '
      
        '                                     Tipo_oper_pedi_vend_tipo.Id' +
        '_Tipopedivend '
      '                                   from '
      '                                     operacao_tipo, '
      '                                     Tipo_oper_pedi_vend_tipo '
      '                                   where '
      
        '                                     operacao_tipo.id_tipooper =' +
        ' Tipo_oper_pedi_vend_tipo.id_tipooper and '
      
        '                                     operacao_tipo.FL_ENTRSAIDTI' +
        'POOPER = '#39'S'#39')'
      
        '  AND PEDIDO_VENDA.DT_ENTRPEDIVEND <= :DT_ENTREGA --TO_DATE('#39'02/' +
        '09/2014'#39','#39'DD/MM/YYYY'#39')  '
      '  AND PEDIDO_VENDA.DT_FATUPEDIVEND IS NULL'
      '  AND PEDIDO_VENDA.ID_CARGEXPE IS NULL'
      '--pedidos sem carga'
      ''
      'ORDER BY 2,6,5'
      ''
      '')
    Left = 270
    Top = 14
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'DT_ENTREGA'
        Value = nil
      end>
    object QrCargasID_CONTA: TFloatField
      FieldName = 'ID_CONTA'
      Required = True
    end
    object QrCargasNM_CONTA: TStringField
      FieldName = 'NM_CONTA'
      Required = True
      Size = 40
    end
    object QrCargasID_PEDIVEND: TFloatField
      FieldName = 'ID_PEDIVEND'
      Required = True
    end
    object QrCargasNM_CLIENTE: TStringField
      FieldName = 'NM_CLIENTE'
      Required = True
      Size = 40
    end
    object QrCargasFL_STATCARGEXPE: TStringField
      FieldName = 'FL_STATCARGEXPE'
      Required = True
      Size = 2
    end
    object QrCargasDT_ENTRPEDIVEND: TDateTimeField
      FieldName = 'DT_ENTRPEDIVEND'
      Required = True
    end
    object QrCargasPESO_TOTAL: TFloatField
      FieldName = 'PESO_TOTAL'
    end
    object QrCargasTOTAL_PEDIDO: TFloatField
      FieldName = 'TOTAL_PEDIDO'
    end
  end
  object QrSQL: TOraQuery
    Session = FrmPrincipal.DB
    Left = 447
    Top = 23
  end
  object QrTotContaAnt: TOraQuery
    Session = FrmPrincipal.DB
    SQL.Strings = (
      'SELECT '
      '  PEDIDO_VENDA.ID_CONTA,'
      '  CONTA.NM_CONTA,'
      '  PEDIDO_VENDA.ID_PEDIVEND,'
      '  SUBSTR(CLIENTE.NM_CLIENTE,1,30) AS NM_CLIENTE,'
      '  PEDIDO_VENDA.DT_ENTRPEDIVEND,'
      '  PEDIDO_VENDA.QN_PESOPEDIVEND,'
      '  PEDIDO_VENDA.VL_PEDIVEND,'
      '  PEDIDO_VENDA.QN_PESOPADREXPEPEDIVEND,'
      '  PEDIDO_VENDA.VL_FATUPEDIVEND'
      'FROM                 '
      '  PEDIDO_VENDA,       '
      '  CONTA,'
      '  EXPEDICAO_CARGA,    '
      '  PEDIDO_VENDA_NOTA_FISCAL, '
      '  NOTA_FISCAL,'
      '  CLIENTE'
      'WHERE          '
      '  PEDIDO_VENDA.ID_CONTA = CONTA.ID_CONTA '
      '  AND (PEDIDO_VENDA.ID_CARGEXPE = EXPEDICAO_CARGA.ID_CARGEXPE)'
      
        '  AND PEDIDO_VENDA_NOTA_FISCAL.ID_PEDIVEND = PEDIDO_VENDA.ID_PED' +
        'IVEND '
      
        '  AND NOTA_FISCAL.ID_NOTAFISC = PEDIDO_VENDA_NOTA_FISCAL.ID_NOTA' +
        'FISC  '
      '  AND PEDIDO_VENDA.ID_CLIENTE = CLIENTE.ID_CLIENTE'
      '  AND NVL(NOTA_FISCAL.FL_CANCNOTAFISC,'#39'N'#39') = '#39'N'#39'  AND '
      '  PEDIDO_VENDA.ID_TIPOPEDIVEND IN (select '
      
        '                                     Tipo_oper_pedi_vend_tipo.Id' +
        '_Tipopedivend '
      '                                   from '
      '                                     operacao_tipo, '
      '                                     Tipo_oper_pedi_vend_tipo '
      '                                   where '
      
        '                                     operacao_tipo.id_tipooper =' +
        ' Tipo_oper_pedi_vend_tipo.id_tipooper and '
      
        '                                     operacao_tipo.FL_ENTRSAIDTI' +
        'POOPER = '#39'S'#39') AND'
      ''
      
        '  PEDIDO_VENDA.DT_ENTRPEDIVEND = TO_DATE('#39'21/08/2014'#39','#39'DD/MM/YYY' +
        'Y'#39')     '
      
        '  AND PEDIDO_VENDA.DT_BLOQAJUSFINAPEDIVEND > TO_DATE('#39'21/08/2014' +
        ' 08:28:41'#39', '#39'DD/MM/YYYY HH24:MI:SS'#39')'
      ''
      'ORDER BY  1,3  '
      '/*'
      'SELECT '
      '  PEDIDO_VENDA.ID_CONTA,'
      '  CONTA.NM_CONTA,'
      '  COUNT(0) as QTDE_PEDIDOS,'
      '  SUM(PEDIDO_VENDA.VL_PEDIVEND) AS TOTAL_PEDIDO,'
      '  SUM(PEDIDO_VENDA.VL_FATUPEDIVEND) AS TOTAL_FATURADO'
      'FROM                 '
      '  PEDIDO_VENDA,       '
      '  CONTA,'
      '  EXPEDICAO_CARGA,    '
      '  PEDIDO_VENDA_NOTA_FISCAL, '
      '  NOTA_FISCAL'
      'WHERE          '
      '  PEDIDO_VENDA.ID_CONTA = CONTA.ID_CONTA '
      '  AND (PEDIDO_VENDA.ID_CARGEXPE = EXPEDICAO_CARGA.ID_CARGEXPE)'
      
        '  AND PEDIDO_VENDA_NOTA_FISCAL.ID_PEDIVEND = PEDIDO_VENDA.ID_PED' +
        'IVEND '
      
        '  AND NOTA_FISCAL.ID_NOTAFISC = PEDIDO_VENDA_NOTA_FISCAL.ID_NOTA' +
        'FISC  '
      '  AND NVL(NOTA_FISCAL.FL_CANCNOTAFISC,'#39'N'#39') = '#39'N'#39'  AND '
      '  PEDIDO_VENDA.ID_TIPOPEDIVEND IN (select '
      
        '                                     Tipo_oper_pedi_vend_tipo.Id' +
        '_Tipopedivend '
      '                                   from '
      '                                     operacao_tipo, '
      '                                     Tipo_oper_pedi_vend_tipo '
      '                                   where '
      
        '                                     operacao_tipo.id_tipooper =' +
        ' Tipo_oper_pedi_vend_tipo.id_tipooper and '
      
        '                                     operacao_tipo.FL_ENTRSAIDTI' +
        'POOPER = '#39'S'#39') AND'
      ''
      
        '  PEDIDO_VENDA.DT_ENTRPEDIVEND = TO_DATE('#39'21/08/2014'#39','#39'DD/MM/YYY' +
        'Y'#39')     '
      
        '  AND PEDIDO_VENDA.DT_BLOQAJUSFINAPEDIVEND > TO_DATE('#39'21/08/2014' +
        ' 08:28:41'#39', '#39'DD/MM/YYYY HH24:MI:SS'#39')'
      'GROUP BY    '
      '  PEDIDO_VENDA.ID_CONTA, '
      '  CONTA.NM_CONTA'
      'ORDER BY  1  '
      '*/')
    Left = 495
    Top = 23
    object QrTotContaAntID_CONTA: TFloatField
      FieldName = 'ID_CONTA'
      Required = True
    end
    object QrTotContaAntNM_CONTA: TStringField
      FieldName = 'NM_CONTA'
      Required = True
      Size = 40
    end
    object QrTotContaAntID_PEDIVEND: TFloatField
      FieldName = 'ID_PEDIVEND'
      Required = True
    end
    object QrTotContaAntNM_CLIENTE: TStringField
      FieldName = 'NM_CLIENTE'
      Required = True
      Size = 40
    end
    object QrTotContaAntDT_ENTRPEDIVEND: TDateTimeField
      FieldName = 'DT_ENTRPEDIVEND'
      Required = True
    end
    object QrTotContaAntQN_PESOPEDIVEND: TFloatField
      FieldName = 'QN_PESOPEDIVEND'
    end
    object QrTotContaAntVL_PEDIVEND: TFloatField
      FieldName = 'VL_PEDIVEND'
    end
    object QrTotContaAntQN_PESOPADREXPEPEDIVEND: TFloatField
      FieldName = 'QN_PESOPADREXPEPEDIVEND'
    end
    object QrTotContaAntVL_FATUPEDIVEND: TFloatField
      FieldName = 'VL_FATUPEDIVEND'
    end
  end
  object QrPedExc: TOraQuery
    Session = FrmPrincipal.DB
    SQL.Strings = (
      'SELECT '
      '  CONTA.ID_CONTA,'
      '  CONTA.NM_CONTA,'
      '  PEDIDO_EXCLUSAO_VENDA.ID_PEDIVEND,'
      '  CLIENTE.NM_CLIENTE,'
      '  PEDIDO_EXCLUSAO_VENDA.DT_ENTRPEDIVEND,'
      '  NVL(PEDIDO_EXCLUSAO_VENDA.QN_PESOPEDIVEND,0) AS PESO_TOTAL,'
      '  NVL(PEDIDO_EXCLUSAO_VENDA.VL_PEDIVEND,0) AS TOTAL_PEDIDO'
      'FROM                 '
      '  PEDIDO_EXCLUSAO_VENDA,       '
      '  CLIENTE,'
      '  CONTA'
      'WHERE          '
      '  PEDIDO_EXCLUSAO_VENDA.ID_CLIENTE = CLIENTE.ID_CLIENTE'
      '  AND PEDIDO_EXCLUSAO_VENDA.ID_CONTA = CONTA.ID_CONTA'
      '  AND PEDIDO_EXCLUSAO_VENDA.ID_TIPOPEDIVEND IN (select '
      
        '                                     Tipo_oper_pedi_vend_tipo.Id' +
        '_Tipopedivend '
      '                                   from '
      '                                     operacao_tipo, '
      '                                     Tipo_oper_pedi_vend_tipo '
      '                                   where '
      
        '                                     operacao_tipo.id_tipooper =' +
        ' Tipo_oper_pedi_vend_tipo.id_tipooper and '
      
        '                                     operacao_tipo.FL_ENTRSAIDTI' +
        'POOPER = '#39'S'#39')'
      
        '  --AND PEDIDO_EXCLUSAO_VENDA.DT_ENTRPEDIVEND = :DT_ENTREGA --TO' +
        '_DATE('#39'03/09/2014'#39','#39'DD/MM/YYYY'#39') '
      ''
      
        '  --AND TRUNC(PEDIDO_EXCLUSAO_VENDA.DT_ENTRPEDIVEND) <= :DT_ENTR' +
        'EGA_ANT'
      
        '  AND TRUNC(PEDIDO_EXCLUSAO_VENDA.DT_ENTRPEDIVEND) = :DT_ENTREGA' +
        '_ANT'
      
        '  --AND TRUNC(PEDIDO_EXCLUSAO_VENDA.DT_EXCLUPEDIVEND) = :DT_EXCL' +
        'USAO'
      ''
      
        '  AND trunc(PEDIDO_EXCLUSAO_VENDA.dt_digipedivend) < trunc(PEDID' +
        'O_EXCLUSAO_VENDA.dt_exclupedivend)')
    Left = 552
    Top = 23
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'DT_ENTREGA_ANT'
        Value = nil
      end>
    object QrPedExcID_CONTA: TFloatField
      FieldName = 'ID_CONTA'
      Required = True
    end
    object QrPedExcNM_CONTA: TStringField
      FieldName = 'NM_CONTA'
      Required = True
      Size = 40
    end
    object QrPedExcID_PEDIVEND: TFloatField
      FieldName = 'ID_PEDIVEND'
    end
    object QrPedExcNM_CLIENTE: TStringField
      FieldName = 'NM_CLIENTE'
      Required = True
      Size = 40
    end
    object QrPedExcDT_ENTRPEDIVEND: TDateTimeField
      FieldName = 'DT_ENTRPEDIVEND'
    end
    object QrPedExcPESO_TOTAL: TFloatField
      FieldName = 'PESO_TOTAL'
    end
    object QrPedExcTOTAL_PEDIDO: TFloatField
      FieldName = 'TOTAL_PEDIDO'
    end
  end
  object DsTotConta: TDataSource
    DataSet = QrTotConta
    Left = 111
    Top = 65
  end
end
