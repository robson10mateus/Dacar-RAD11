object FrmOcorrencias: TFrmOcorrencias
  Left = 477
  Top = 239
  Width = 1040
  Height = 650
  VertScrollBar.Range = 45
  ActiveControl = PageControl1
  BorderStyle = bsSingle
  Caption = 'Comercial Dacar - Chamados e Ocorr'#234'ncias'
  Color = clWindow
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -12
  Font.Name = 'Segoe UI'
  Font.Style = []
  FormStyle = fsMDIChild
  Icon.Data = {
    
    0000000000000000000000000000}
  Position = poDefault
  Visible = True
  WindowState = wsMaximized
  OnClose = FormClose
  OnShow = FormShow
  TextHeight = 15
  object PageControl1: TPageControl
    Left = 0
    Top = 45
    Width = 1028
    Height = 567
    ActivePage = Tab_Detalhe
    Align = alClient
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlack
    Font.Height = 15
    Font.Name = 'Arial'
    Font.Pitch = fpVariable
    Font.Style = []
    ParentFont = False
    TabOrder = 0
    ExplicitWidth = 1024
    ExplicitHeight = 566
    object Tab_Criterio: TTabSheet
      Caption = ' Crit'#233'rio '
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = 15
      Font.Name = 'Arial'
      Font.Pitch = fpVariable
      Font.Style = []
      ParentFont = False
      object Label6: TLabel
        Left = 270
        Top = 27
        Width = 53
        Height = 15
        Caption = 'Condi'#231#227'o'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = 15
        Font.Name = 'Arial'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
      end
      object Label7: TLabel
        Left = 270
        Top = 67
        Width = 59
        Height = 15
        Caption = 'Express'#227'o'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = 15
        Font.Name = 'Arial'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
      end
      object LCampos: TListBox
        Left = 10
        Top = 10
        Width = 250
        Height = 300
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = 15
        Font.Name = 'Arial'
        Font.Pitch = fpVariable
        Font.Style = []
        ItemHeight = 15
        ParentFont = False
        TabOrder = 0
        OnClick = LCamposClick
      end
      object CBCond: TComboBox
        Left = 340
        Top = 24
        Width = 138
        Height = 23
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = 15
        Font.Name = 'Arial'
        Font.Pitch = fpVariable
        Font.Style = []
        ItemIndex = 0
        ParentFont = False
        TabOrder = 1
        Text = 'Igual a'
        Items.Strings = (
          'Igual a'
          'Diferente de'
          'Maior que'
          'Maior ou igual a'
          'Menor que'
          'Menor ou igual a'
          'Contendo')
      end
      object RGeou: TRadioGroup
        Left = 540
        Top = 24
        Width = 105
        Height = 25
        Columns = 2
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = 13
        Font.Name = 'Arial'
        Font.Pitch = fpVariable
        Font.Style = [fsBold]
        ItemIndex = 0
        Items.Strings = (
          'E'
          'OU')
        ParentFont = False
        TabOrder = 2
      end
      object EdExpre: TEdit
        Left = 340
        Top = 65
        Width = 300
        Height = 23
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = 15
        Font.Name = 'Arial'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 3
        OnChange = EdExpreChange
      end
      object MResul: TMemo
        Left = 270
        Top = 110
        Width = 468
        Height = 200
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = 15
        Font.Name = 'Arial'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 4
      end
      object BtIncluir: TBitBtn
        Left = 663
        Top = 17
        Width = 75
        Height = 23
        Caption = 'Incluir'
        Default = True
        Enabled = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = 16
        Font.Name = 'Arial'
        Font.Pitch = fpVariable
        Font.Style = []
        Glyph.Data = {
          
          0000}
        ParentFont = False
        TabOrder = 6
        OnClick = BtIncluirClick
      end
      object BtLimpar: TBitBtn
        Left = 663
        Top = 46
        Width = 75
        Height = 23
        Caption = 'Limpar'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = 16
        Font.Name = 'Arial'
        Font.Pitch = fpVariable
        Font.Style = []
        Glyph.Data = {
          
          0000FFFFF70007FFFFFFF0000000}
        ParentFont = False
        TabOrder = 8
        OnClick = BtLimparClick
      end
      object BtAtivar: TBitBtn
        Left = 664
        Top = 75
        Width = 75
        Height = 22
        Caption = 'Ativar'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = 16
        Font.Name = 'Arial'
        Font.Pitch = fpVariable
        Font.Style = []
        Glyph.Data = {
          
          FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
        ParentFont = False
        TabOrder = 5
        OnClick = BtAtivarClick
      end
      object BPESQ: TBitBtn
        Left = 620
        Top = 65
        Width = 23
        Height = 24
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = 16
        Font.Name = 'Arial'
        Font.Pitch = fpVariable
        Font.Style = []
        Glyph.Data = {
          
          33333333333333333333}
        ParentFont = False
        TabOrder = 7
        Visible = False
        OnClick = BPESQClick
      end
    end
    object Tab_Lista: TTabSheet
      Caption = ' Lista '
      ImageIndex = 1
      object DBGrid1: TDBGrid
        Left = 0
        Top = 0
        Width = 1020
        Height = 537
        Align = alClient
        DataSource = Ds
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = 15
        Font.Name = 'Arial'
        Font.Pitch = fpVariable
        Font.Style = []
        Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit, dgMultiSelect]
        ParentFont = False
        TabOrder = 0
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clBlack
        TitleFont.Height = 16
        TitleFont.Name = 'Arial'
        TitleFont.Pitch = fpVariable
        TitleFont.Style = [fsBold]
        OnDrawColumnCell = DBGrid1DrawColumnCell
        OnTitleClick = DBGrid1TitleClick
        Columns = <
          item
            Expanded = False
            FieldName = 'ID_CHAMADOS'
            Title.Caption = 'C'#243'd'
            Width = 50
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'DT_CHAMADOS'
            Title.Caption = 'Data'
            Width = 100
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'OCORRENCIA'
            Title.Caption = 'Ocorr'#234'ncia'
            Width = 350
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'OBS_CHAMADOS'
            Title.Caption = 'Observa'#231#227'o'
            Width = 400
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'FL_CHAMADOS'
            Title.Caption = 'Resolvido'
            Visible = True
          end>
      end
    end
    object Tab_Detalhe: TTabSheet
      Caption = ' Detalhe '
      ImageIndex = 3
      object Label14: TLabel
        Left = 8
        Top = 60
        Width = 60
        Height = 15
        Caption = 'Ocorr'#234'ncia'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = 15
        Font.Name = 'Arial'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
      end
      object Label1: TLabel
        Left = 520
        Top = 60
        Width = 88
        Height = 15
        Caption = 'Medida Tomada'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = 15
        Font.Name = 'Arial'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
      end
      object Label2: TLabel
        Left = 10
        Top = 185
        Width = 66
        Height = 15
        Caption = 'Observa'#231#227'o'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = 15
        Font.Name = 'Arial'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
      end
      object Label3: TLabel
        Left = 520
        Top = 185
        Width = 118
        Height = 15
        Caption = 'Observa'#231#245'es Dept. TI'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = 15
        Font.Name = 'Arial'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
      end
      object Label4: TLabel
        Left = 75
        Top = 10
        Width = 106
        Height = 15
        Caption = 'Data da Ocorr'#234'ncia'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = 15
        Font.Name = 'Arial'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
      end
      object Label5: TLabel
        Left = 200
        Top = 10
        Width = 117
        Height = 15
        Caption = 'Usu'#225'rio Requisitante'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = 15
        Font.Name = 'Arial'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
      end
      object Label8: TLabel
        Left = 8
        Top = 10
        Width = 60
        Height = 16
        AutoSize = False
        Caption = 'C'#243'digo'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = 15
        Font.Name = 'Arial'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
      end
      object Edt_DtOcorrencia: TDBEdit
        Left = 76
        Top = 30
        Width = 115
        Height = 23
        Color = clWhite
        DataField = 'DT_CHAMADOS'
        DataSource = Ds
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = 15
        Font.Name = 'Arial'
        Font.Pitch = fpVariable
        Font.Style = []
        MaxLength = 7
        ParentFont = False
        TabOrder = 2
      end
      object btn_DtEntrega: TBitBtn
        Left = 171
        Top = 32
        Width = 20
        Height = 20
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = 16
        Font.Name = 'Arial'
        Font.Pitch = fpVariable
        Font.Style = []
        Glyph.Data = {
          
          33333333333333333333}
        ParentFont = False
        TabOrder = 5
        OnClick = btn_DtEntregaClick
      end
      object edtNomeUsuario: TDBEdit
        Left = 200
        Top = 30
        Width = 300
        Height = 23
        Color = clSilver
        DataField = 'NOME_USUARIO'
        DataSource = Ds
        Enabled = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = 15
        Font.Name = 'Arial'
        Font.Pitch = fpVariable
        Font.Style = []
        MaxLength = 7
        ParentFont = False
        TabOrder = 7
      end
      object DBEdit1: TDBEdit
        Left = 8
        Top = 30
        Width = 60
        Height = 24
        AutoSize = False
        Color = clSilver
        DataField = 'ID_CHAMADOS'
        DataSource = Ds
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = 15
        Font.Name = 'Arial'
        Font.Pitch = fpVariable
        Font.Style = []
        MaxLength = 7
        ParentFont = False
        ReadOnly = True
        TabOrder = 0
      end
      object MemOcorrencia: TDBMemo
        Left = 8
        Top = 80
        Width = 500
        Height = 100
        DataField = 'OCORRENCIA'
        DataSource = Ds
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = 15
        Font.Name = 'Arial'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 1
        OnKeyPress = MemOcorrenciaKeyPress
      end
      object MemMedida: TDBMemo
        Left = 520
        Top = 80
        Width = 500
        Height = 100
        DataField = 'MED_CHAMADOS'
        DataSource = Ds
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = 15
        Font.Name = 'Arial'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 3
        OnKeyPress = MemMedidaKeyPress
      end
      object MemObs: TDBMemo
        Left = 10
        Top = 210
        Width = 500
        Height = 100
        DataField = 'OBS_CHAMADOS'
        DataSource = Ds
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = 15
        Font.Name = 'Arial'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 4
        OnKeyPress = MemObsKeyPress
      end
      object MemObs2: TDBMemo
        Left = 520
        Top = 210
        Width = 500
        Height = 100
        DataField = 'OBS_CHAMADOS2'
        DataSource = Ds
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = 15
        Font.Name = 'Arial'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 6
        OnKeyPress = MemObs2KeyPress
      end
      object rgpFlag: TDBRadioGroup
        Left = 520
        Top = 5
        Width = 150
        Height = 50
        Caption = ' Resolvido '
        Columns = 2
        DataField = 'FL_CHAMADOS'
        DataSource = Ds
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = 15
        Font.Name = 'Arial'
        Font.Pitch = fpVariable
        Font.Style = []
        Items.Strings = (
          'Sim'
          'N'#227'o')
        ParentFont = False
        TabOrder = 8
        Values.Strings = (
          'S'
          'N')
      end
    end
  end
  object pnlBotoes: TPanel
    Left = 0
    Top = 0
    Width = 1028
    Height = 45
    Align = alTop
    Color = clWindow
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = 15
    Font.Name = 'Arial'
    Font.Style = []
    ParentBackground = False
    ParentFont = False
    TabOrder = 1
    ExplicitWidth = 1024
    object SB_ANTERIOR: TSpeedButton
      Left = 5
      Top = 4
      Width = 35
      Height = 35
      Hint = 'Anterior'
      Enabled = False
      Flat = True
      Glyph.Data = {
        
        D3F7F7F7FFFFFFFFFFFF}
      NumGlyphs = 2
      ParentShowHint = False
      ShowHint = False
      OnClick = SB_ANTERIORClick
    end
    object SB_PROXIMO: TSpeedButton
      Left = 40
      Top = 4
      Width = 35
      Height = 35
      Hint = 'Pr'#243'ximo'
      Enabled = False
      Flat = True
      Glyph.Data = {
        
        E5E5E5E5E5E5E5E5E4E4E4E6E6E6E6E6E6F0F0F0FFFFFFFFFFFF}
      NumGlyphs = 2
      ParentShowHint = False
      ShowHint = False
      OnClick = SB_PROXIMOClick
    end
    object SBNovo: TSpeedButton
      Left = 75
      Top = 4
      Width = 35
      Height = 35
      Hint = 'Novo Vale'
      Enabled = False
      Flat = True
      Glyph.Data = {
        
        5050505D5D5D959595FFFFFFFFFFFFFDFDFD}
      NumGlyphs = 2
      ParentShowHint = False
      ShowHint = False
      OnClick = SBNovoClick
    end
    object SB_Editar: TSpeedButton
      Left = 145
      Top = 4
      Width = 35
      Height = 35
      Hint = 'Editar'
      Enabled = False
      Flat = True
      Glyph.Data = {
        
        C3C3C3C3C3C5C5C5C4C4C4C0C0C0D0D0D0E9E9E9FDFDFDFDFDFD}
      NumGlyphs = 2
      ParentShowHint = False
      ShowHint = False
      OnClick = SB_EditarClick
    end
    object SBDel: TSpeedButton
      Left = 110
      Top = 4
      Width = 35
      Height = 35
      Hint = 'Excluir'
      Enabled = False
      Flat = True
      Glyph.Data = {
        
        0000}
      NumGlyphs = 2
      ParentShowHint = False
      ShowHint = False
      Visible = False
      OnClick = SBDelClick
    end
    object SBSalva: TSpeedButton
      Left = 180
      Top = 4
      Width = 35
      Height = 35
      Hint = 'Confirmar'
      Enabled = False
      Flat = True
      Glyph.Data = {
        
        7D7D7D7D7D7D7D7D7D7C7C7C818181929292909090E3E3E30000}
      NumGlyphs = 2
      ParentShowHint = False
      ShowHint = False
      OnClick = SBSalvaClick
    end
    object SBCancel: TSpeedButton
      Left = 215
      Top = 4
      Width = 35
      Height = 35
      Hint = 'Cancelar'
      Enabled = False
      Flat = True
      Glyph.Data = {
        
        7E7E7E7B7B7B7B7B7B757575959595F5F5F5}
      NumGlyphs = 2
      ParentShowHint = False
      ShowHint = False
      OnClick = SBCancelClick
    end
    object Sb_Sair: TSpeedButton
      Left = 250
      Top = 4
      Width = 35
      Height = 35
      Hint = 'Sair'
      Flat = True
      Glyph.Data = {
        
        FEFEFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF00}
      ParentShowHint = False
      ShowHint = True
      OnClick = Sb_SairClick
    end
    object SpeedButton1: TSpeedButton
      Left = 442
      Top = 4
      Width = 107
      Height = 30
      Caption = 'Email'
      Flat = True
      Visible = False
      OnClick = SpeedButton1Click
    end
  end
  object Ds: TOraDataSource
    DataSet = Qr
    OnDataChange = DsDataChange
    Left = 334
    Top = 7
  end
  object Qr_Direitos: TOraQuery
    Session = FrmPrincipal.DB
    SQL.Strings = (
      'Select MAX(ID_RECURSO) ID_RECURSO'
      '  From Sct_Direitos_Us'
      ' Where ID_Modulo = 9 '
      '   And ID_JANELA = 59'
      '   And ID_RECURSO in ( 4,5 )'
      '   AND ID_Usuario = :ID_Usuario')
    Left = 307
    Top = 7
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'ID_Usuario'
        Value = nil
      end>
    object Qr_DireitosID_RECURSO: TFloatField
      FieldName = 'ID_RECURSO'
    end
  end
  object QryTemp: TOraQuery
    Left = 365
    Top = 7
  end
  object Qr: TSmartQuery
    KeyFields = 'ID_CHAMADOS'
    KeySequence = 'SMART.ID_CHAMADOS_DACAR_CHAMADOS'
    SQLInsert.Strings = (
      'INSERT INTO DACAR_CHAMADOS'
      
        '  (ID_CHAMADOS, DT_CHAMADOS, OCORRENCIA, MED_CHAMADOS, OBS_CHAMA' +
        'DOS, OBS_CHAMADOS2, FL_CHAMADOS, ID_USUARIO, EMPRESA, FILIAL) '
      'VALUES'
      
        '  (:ID_CHAMADOS, :DT_CHAMADOS, :OCORRENCIA, :MED_CHAMADOS, :OBS_' +
        'CHAMADOS, :OBS_CHAMADOS2, :FL_CHAMADOS, :ID_USUARIO, :EMPRESA, :' +
        'FILIAL)')
    SQLDelete.Strings = (
      'DELETE FROM DACAR_CHAMADOS'
      'WHERE'
      '  ID_CHAMADOS = :ID_CHAMADOS')
    SQLUpdate.Strings = (
      'UPDATE DACAR_CHAMADOS'
      'SET'
      '  ID_CHAMADOS       = :ID_CHAMADOS,'
      '  DT_CHAMADOS       = :DT_CHAMADOS,'
      '  OCORRENCIA        = :OCORRENCIA,'
      '  MED_CHAMADOS      = :MED_CHAMADOS,'
      '  OBS_CHAMADOS      = :OBS_CHAMADOS,'
      '  OBS_CHAMADOS2     = :OBS_CHAMADOS2,'
      '  FL_CHAMADOS       = :FL_CHAMADOS,'
      '  ID_USUARIO        = :ID_USUARIO,'
      '  EMPRESA           = :EMPRESA,'
      '  FILIAL            = :FILIAL    '
      'WHERE'
      '  ID_CHAMADOS = :OLD_ID_CHAMADOS')
    SQLRefresh.Strings = (
      'WHERE'
      '  DC.ID_CHAMADOS = :ID_CHAMADOS')
    SQLLock.Strings = (
      'SELECT * FROM DACAR_CHAMADOS'
      'WHERE'
      '  ID_CHAMADOS = :ID_CHAMADOS'
      'FOR UPDATE NOWAIT')
    Session = FrmPrincipal.DB
    SQL.Strings = (
      'SELECT DC.EMPRESA,'
      '       DC.FILIAL,'
      '       DC.ID_CHAMADOS,'
      '       DC.DT_CHAMADOS,'
      '       DC.OCORRENCIA,'
      '       DC.MED_CHAMADOS,'
      '       DC.OBS_CHAMADOS,'
      '       DC.OBS_CHAMADOS2,'
      '       DC.FL_CHAMADOS,'
      '       U.ID_USUARIO,'
      '       U.NOME_USUARIO'
      'FROM DACAR_CHAMADOS DC'
      'LEFT JOIN SCT_USUARIO U ON DC.ID_USUARIO = U.ID_USUARIO'
      ''
      '&Macro'
      ''
      'ORDER BY DC.DT_CHAMADOS DESC, DC.ID_CHAMADOS ')
    CachedUpdates = True
    LockMode = lmNone
    Options.SetFieldsReadOnly = False
    Options.ExtendedFieldsInfo = False
    Left = 392
    Top = 8
    MacroData = <
      item
        Name = 'Macro'
      end>
    object QrEMPRESA: TStringField
      FieldName = 'EMPRESA'
      Size = 2
    end
    object QrFILIAL: TIntegerField
      FieldName = 'FILIAL'
    end
    object QrID_CHAMADOS: TIntegerField
      FieldName = 'ID_CHAMADOS'
      Required = True
    end
    object QrDT_CHAMADOS: TDateTimeField
      FieldName = 'DT_CHAMADOS'
    end
    object QrOCORRENCIA: TStringField
      FieldName = 'OCORRENCIA'
      Size = 500
    end
    object QrMED_CHAMADOS: TStringField
      FieldName = 'MED_CHAMADOS'
      Size = 500
    end
    object QrOBS_CHAMADOS: TStringField
      FieldName = 'OBS_CHAMADOS'
      Size = 500
    end
    object QrOBS_CHAMADOS2: TStringField
      FieldName = 'OBS_CHAMADOS2'
      Size = 500
    end
    object QrFL_CHAMADOS: TStringField
      FieldName = 'FL_CHAMADOS'
      Size = 1
    end
    object QrID_USUARIO: TIntegerField
      FieldName = 'ID_USUARIO'
    end
    object QrNOME_USUARIO: TStringField
      FieldName = 'NOME_USUARIO'
      Size = 40
    end
  end
end
