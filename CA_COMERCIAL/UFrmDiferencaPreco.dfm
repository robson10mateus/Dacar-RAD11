object FrmDiferencaPreco: TFrmDiferencaPreco
  Left = 557
  Top = 108
    ...
    object Tab_Lista: TTabSheet
      ...
      object DBGrid1: TDBGrid
        ...
        Columns = <
          ...
          item
            Expanded = False
            FieldName = 'TOTAL_TABELA'
            Title.Caption = 'Total Tabela'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'UNIT_VENDA'
            Title.Caption = 'Vl Unit Venda'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'UNIT_TABELA'
            Title.Caption = 'Vl Unit Tabela'
            Visible = True
          end>
      end
    end
  ...
  object Qr: TOraQuery
    ...
    SQL.Strings = (
      'SELECT --PT.ID_TIPOPEDIVEND,'
      '       --PT.NM_TIPOPEDIVEND, '
      
        '       PV.ID_VENDEDOR, V.NM_VENDEDOR, ME.ID_PRODMATEEMBA, ME.NM_' +
        'PRODMATEEMBA, NF.DT_EMISNOTAFISC, NF.DT_SAIDNOTAFISC, PV.ID_PEDI' +
        'VEND, PV.ID_CLIENTE, C.NM_CLIENTE, PI.QN_PESOPADREXPEITEMPEDIVEN' +
        'D Quantidade ,'
      
        '       Round(PI.QN_PESOPADREXPEITEMPEDIVEND*PI.VL_UNITITEMPEDIVE' +
        'ND,2)  AS Total_Venda,'
      
        '       Round(PI.QN_PESOPADREXPEITEMPEDIVEND*PI.VL_ITEMTABEPREC,2' +
        ')  AS TOTAL_TABELA,'
        ...
        '       Round(PI.QN_PESOPADREXPEITEMPEDIVEND*PI.VL_ITEMTABEPREC,2' +
        ')  AS DIF_TOTAL,'
      '       Round(PI.VL_UNITITEMPEDIVEND,2)  UNIT_VENDA,'
      '       Round(PI.VL_ITEMTABEPREC,2)      UNIT_TABELA,'
      
        '       Round(PI.VL_UNITITEMPEDIVEND,2) - Round(PI.VL_ITEMTABEPRE' +
        'C,2) AS DIF_UNIT,'
      '       NF.NR_NOTAFISC       '
      '  FROM PEDIDO_VENDA PV                                  '
      ' INNER JOIN PEDIDO_VENDA_TIPO PT                             '
      '    ON (PT.ID_TIPOPEDIVEND = PV.ID_TIPOPEDIVEND)'
      ' INNER JOIN VENDEDOR V                                      '
      '    ON (V.ID_VENDEDOR = PV.ID_VENDEDOR)'
      '-- FABRICIO: 03/01/18 - DIF PRECO SOMENTE QND NAO TEM % COMISS'
      
        ' INNER JOIN CLIENTE_VENDEDOR CV                                 ' +
        '     '
      '    ON (CV.ID_VENDEDOR = PV.ID_VENDEDOR'
      '    AND CV.ID_CLIENTE = PV.ID_CLIENTE)    '
      ' INNER JOIN CLIENTE C                                       '
      '   ON  (C.ID_CLIENTE = PV.ID_CLIENTE)                          '
      ' INNER JOIN PEDIDO_VENDA_ITEM PI                             '
      '    ON (PI.ID_PEDIVEND = PV.ID_PEDIVEND)              '
      ' INNER JOIN MATERIAL_EMBALAGEM ME '
      '    ON (PI.ID_MATEEMBA = ME.ID_MATEEMBA'
      '    AND PI.ID_MATERIAL = ME.ID_MATERIAL)'
      ' INNER JOIN PEDIDO_VENDA_NOTA_FISCAL PN'
      '    ON (PN.ID_PEDIVEND = PV.ID_PEDIVEND)'
      ' INNER JOIN NOTA_FISCAL NF'
      '    ON (NF.ID_NOTAFISC = PN.ID_NOTAFISC)'
      'WHERE PT.FL_GERACOMITIPOPEDIVEND = '#39'S'#39
      '  AND NVL(NF.FL_CANCNOTAFISC,'#39'N'#39') <> '#39'S'#39
      '  --FABRICIO: 03/01/18 - DIF PRECO SOMENTE QND NAO TEM % COMISS'
      '  AND CV.PC_COMIVENDCLIE = 0'
      ''
      '&Macro'
      ''
      '&MacroData'
      ''
        ...
        'ORDER BY V.NM_VENDEDOR , ME.ID_PRODMATEEMBA, NF.DT_EMISNOTAFISC,' +
        'NF.DT_SAIDNOTAFISC')
    FetchAll = True
    Left = 112
    Top = 104
    MacroData = <
      item
        Name = 'Macro'
        Value = 'AND PV.ID_VENDEDOR = 14'
      end
      item
        Name = 'MacroData'
        Value = 
          '  AND TRUNC( NF.DT_EMISNOTAFISC ) >= TO_DATE('#39'01/09/2017'#39','#39'DD/MM' +
          '/YYYY'#39') AND '#13#10'     TRUNC( NF.DT_EMISNOTAFISC ) <= TO_DATE('#39'06/09' +
          '/2017'#39','#39'DD/MM/YYYY'#39' )'
      end>
    ...
    object QrQUANTIDADE: TFloatField
      FieldName = 'QUANTIDADE'
      Required = True
      DisplayFormat = '#,##0.00'
    end
    object QrTOTAL_VENDA: TFloatField
      FieldName = 'TOTAL_VENDA'
      DisplayFormat = '#,##0.00'
    end
    object QrTOTAL_TABELA: TFloatField
      FieldName = 'TOTAL_TABELA'
      DisplayFormat = '#,##0.00'
    end
    object QrUNIT_VENDA: TFloatField
      FieldName = 'UNIT_VENDA'
      Required = True
      DisplayFormat = '#,##0.00'
    end
    object QrUNIT_TABELA: TFloatField
      FieldName = 'UNIT_TABELA'
      DisplayFormat = '#,##0.00'
    end
    ...
    object QrDIF_TOTAL: TFloatField
      FieldName = 'DIF_TOTAL'
      DisplayFormat = '#,##0.00'
    end
    object QrDIF_UNIT: TFloatField
      FieldName = 'DIF_UNIT'
      DisplayFormat = '#,##0.00'
    end
  end
