object FrmUltVend: TFrmUltVend
  Left = 188
  Top = 82
  Width = 519
  Height = 453
  VertScrollBar.Range = 41
  ActiveControl = PageControl1
  BorderIcons = [biSystemMenu, biMinimize, biMaximize, biHelp]
  BorderStyle = bsSingle
  Caption = #218'ltima Venda'
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -12
  Font.Name = 'Segoe UI'
  Font.Style = []
  OnClose = FormClose
  TextHeight = 15
  object ToolBar1: TToolBar
    Left = 0
    Top = 0
    Width = 503
    Height = 41
    Anchors = []
    ButtonHeight = 37
    ButtonWidth = 39
    Caption = 'ToolBar1'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = 11
    Font.Name = 'helvetica'
    Font.Pitch = fpVariable
    Font.Style = []
    ParentFont = False
    TabOrder = 0
    ExplicitWidth = 507
    object Sb_Sair: TSpeedButton
      Left = 0
      Top = 0
      Width = 40
      Height = 37
      Hint = 'Sair'
      Flat = True
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF0000}
      NumGlyphs = 2
      ParentShowHint = False
      ShowHint = True
      OnClick = Sb_SairClick
    end
    object ToolButton3: TToolButton
      Left = 40
      Top = 0
      Width = 474
      Caption = 'ToolButton3'
      ImageIndex = 1
      Style = tbsSeparator
    end
  end
  object PageControl1: TPageControl
    Left = 0
    Top = 41
    Width = 503
    Height = 373
    ActivePage = TabSheet1
    Align = alClient
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlack
    Font.Height = 15
    Font.Name = 'Arial'
    Font.Pitch = fpVariable
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 1
    object TabSheet1: TTabSheet
      Caption = 'Detalhe'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = 11
      Font.Name = 'helvetica'
      Font.Pitch = fpVariable
      Font.Style = [fsBold]
      ParentFont = False
      object DBGrid1: TDBGrid
        Left = 0
        Top = 177
        Width = 499
        Height = 167
        Align = alClient
        Color = clWhite
        DataSource = DSItem
        FixedColor = clSilver
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = 15
        Font.Name = 'Arial'
        Font.Pitch = fpVariable
        Font.Style = []
        Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgConfirmDelete, dgCancelOnExit, dgMultiSelect]
        ParentFont = False
        ReadOnly = True
        TabOrder = 0
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clBlack
        TitleFont.Height = 16
        TitleFont.Name = 'Arial'
        TitleFont.Pitch = fpVariable
        TitleFont.Style = [fsBold]
        Columns = <
          item
            Expanded = False
            FieldName = 'ID_PRODMATEEMBA'
            Title.Caption = 'C'#243'digo'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'NM_PRODMATEEMBA'
            Title.Caption = 'Descri'#231#227'o do Produto'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'QN_EMBAITEMPEDIVEND'
            Title.Caption = 'Qtde. Embal.'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'QN_PESOITEMPEDIVEND'
            Title.Caption = 'Peso'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'VL_UNITITEMPEDIVEND'
            Title.Caption = 'Vl. Unit'#225'rio'
            Visible = True
          end>
      end
      object Panel1: TPanel
        Left = 0
        Top = 0
        Width = 499
        Height = 177
        Align = alTop
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = 15
        Font.Name = 'Arial'
        Font.Pitch = fpVariable
        Font.Style = [fsBold]
        ParentBackground = False
        ParentFont = False
        TabOrder = 1
        ExplicitWidth = 495
        object Label1: TLabel
          Left = 32
          Top = 17
          Width = 39
          Height = 15
          Caption = 'C'#243'digo'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = 15
          Font.Name = 'Arial'
          Font.Pitch = fpVariable
          Font.Style = [fsBold]
          ParentFont = False
        end
        object Label2: TLabel
          Left = 144
          Top = 17
          Width = 39
          Height = 15
          Caption = 'Cliente'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = 15
          Font.Name = 'Arial'
          Font.Pitch = fpVariable
          Font.Style = [fsBold]
          ParentFont = False
        end
        object Label3: TLabel
          Left = 32
          Top = 65
          Width = 39
          Height = 15
          Caption = 'Pedido'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = 15
          Font.Name = 'Arial'
          Font.Pitch = fpVariable
          Font.Style = [fsBold]
          ParentFont = False
        end
        object Label4: TLabel
          Left = 144
          Top = 65
          Width = 56
          Height = 15
          Caption = 'Vl. Pedido'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = 15
          Font.Name = 'Arial'
          Font.Pitch = fpVariable
          Font.Style = [fsBold]
          ParentFont = False
        end
        object Label5: TLabel
          Left = 256
          Top = 65
          Width = 68
          Height = 15
          Caption = 'Data Pedido'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = 15
          Font.Name = 'Arial'
          Font.Pitch = fpVariable
          Font.Style = [fsBold]
          ParentFont = False
        end
        object Label6: TLabel
          Left = 368
          Top = 65
          Width = 73
          Height = 15
          Caption = 'Data Entrega'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = 15
          Font.Name = 'Arial'
          Font.Pitch = fpVariable
          Font.Style = [fsBold]
          ParentFont = False
        end
        object Label7: TLabel
          Left = 32
          Top = 113
          Width = 121
          Height = 15
          Caption = 'Forma de Pagamento'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = 15
          Font.Name = 'Arial'
          Font.Pitch = fpVariable
          Font.Style = [fsBold]
          ParentFont = False
        end
        object Label8: TLabel
          Left = 368
          Top = 113
          Width = 96
          Height = 15
          Caption = 'Data Vencimento'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = 15
          Font.Name = 'Arial'
          Font.Pitch = fpVariable
          Font.Style = [fsBold]
          ParentFont = False
        end
        object DBEdit1: TDBEdit
          Left = 28
          Top = 33
          Width = 101
          Height = 23
          DataField = 'ID_CLIENTE'
          DataSource = DSUltVend
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = 15
          Font.Name = 'Arial'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          TabOrder = 0
        end
        object DBEdit2: TDBEdit
          Left = 140
          Top = 33
          Width = 325
          Height = 23
          DataField = 'NM_CLIENTE'
          DataSource = DSUltVend
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = 15
          Font.Name = 'Arial'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          TabOrder = 1
        end
        object DBEdit3: TDBEdit
          Left = 28
          Top = 81
          Width = 101
          Height = 23
          DataField = 'ID_PEDIVEND'
          DataSource = DSUltVend
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = 15
          Font.Name = 'Arial'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          TabOrder = 2
        end
        object DBEdit4: TDBEdit
          Left = 140
          Top = 81
          Width = 101
          Height = 23
          DataField = 'VL_PEDIVEND'
          DataSource = DSUltVend
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = 15
          Font.Name = 'Arial'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          TabOrder = 3
        end
        object DBEdit5: TDBEdit
          Left = 252
          Top = 81
          Width = 101
          Height = 23
          DataField = 'DT_PEDIVEND'
          DataSource = DSUltVend
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = 15
          Font.Name = 'Arial'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          TabOrder = 4
        end
        object DBEdit6: TDBEdit
          Left = 364
          Top = 81
          Width = 101
          Height = 23
          DataField = 'DT_ENTRPEDIVEND'
          DataSource = DSUltVend
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = 15
          Font.Name = 'Arial'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          TabOrder = 5
        end
        object DBEdit7: TDBEdit
          Left = 28
          Top = 128
          Width = 325
          Height = 23
          DataField = 'NM_FORMPAGA'
          DataSource = DSUltVend
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = 15
          Font.Name = 'Arial'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          TabOrder = 6
        end
        object DBEdit8: TDBEdit
          Left = 364
          Top = 128
          Width = 101
          Height = 23
          DataField = 'DT_VENCPEDIVEND'
          DataSource = DSUltVend
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = 15
          Font.Name = 'Arial'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          TabOrder = 7
        end
      end
    end
  end
  object QRUltVend: TOraQuery
    Session = FrmPrincipal.DB
    SQL.Strings = (
      'SELECT CLIENTE.ID_CLIENTE,'
      '       CLIENTE.NM_CLIENTE,'
      '       PEDIDO_VENDA.ID_PEDIVEND,'
      '       PEDIDO_VENDA.DT_PEDIVEND,'
      '       PEDIDO_VENDA.DT_ENTRPEDIVEND,'
      '       DECODE(PEDIDO_VENDA.DT_FATUPEDIVEND,'
      
        '              NULL, PEDIDO_VENDA.DT_PEDIVEND + FORMA_PAGAMENTO_P' +
        'ARCELA.QN_DIASPARCFORMPAGA,'
      
        '                    PEDIDO_VENDA.DT_FATUPEDIVEND + FORMA_PAGAMEN' +
        'TO_PARCELA.QN_DIASPARCFORMPAGA) DT_VENCPEDIVEND,'
      '       PEDIDO_VENDA.VL_PEDIVEND,'
      '       FORMA_PAGAMENTO.NM_FORMPAGA'
      '  FROM CLIENTE,'
      '       PEDIDO_VENDA,'
      '       FORMA_PAGAMENTO,'
      '       FORMA_PAGAMENTO_PARCELA'
      ' WHERE PEDIDO_VENDA.ID_CLIENTE = CLIENTE.ID_CLIENTE'
      '   AND PEDIDO_VENDA.ID_FORMPAGA = FORMA_PAGAMENTO.ID_FORMPAGA'
      
        '   AND FORMA_PAGAMENTO_PARCELA.ID_FORMPAGA = FORMA_PAGAMENTO.ID_' +
        'FORMPAGA'
      '   AND PEDIDO_VENDA.ID_PEDIVEND = (  SELECT MAX(ID_PEDIVEND) '
      '                                       FROM PEDIDO_VENDA'
      
        '                                      WHERE ID_CLIENTE = :pi_IdC' +
        'lie)'
      ''
      ''
      '')
    Left = 240
    Top = 24
    ParamData = <
      item
        DataType = ftFloat
        Name = 'pi_IdClie'
        ParamType = ptInput
        Value = nil
      end>
    object QRUltVendID_CLIENTE: TFloatField
      FieldName = 'ID_CLIENTE'
      Required = True
    end
    object QRUltVendNM_CLIENTE: TStringField
      FieldName = 'NM_CLIENTE'
      Required = True
      Size = 40
    end
    object QRUltVendID_PEDIVEND: TFloatField
      FieldName = 'ID_PEDIVEND'
      Required = True
    end
    object QRUltVendDT_PEDIVEND: TDateTimeField
      FieldName = 'DT_PEDIVEND'
      Required = True
    end
    object QRUltVendDT_ENTRPEDIVEND: TDateTimeField
      FieldName = 'DT_ENTRPEDIVEND'
      Required = True
    end
    object QRUltVendDT_VENCPEDIVEND: TDateTimeField
      FieldName = 'DT_VENCPEDIVEND'
    end
    object QRUltVendVL_PEDIVEND: TFloatField
      FieldName = 'VL_PEDIVEND'
      currency = True
    end
    object QRUltVendNM_FORMPAGA: TStringField
      FieldName = 'NM_FORMPAGA'
      Required = True
      Size = 40
    end
  end
  object DSUltVend: TOraDataSource
    DataSet = QRUltVend
    Left = 280
    Top = 24
  end
  object QRItemVend: TOraQuery
    Session = FrmPrincipal.DB
    SQL.Strings = (
      'SELECT PEDIDO_VENDA_ITEM.QN_EMBAITEMPEDIVEND,'
      '       PEDIDO_VENDA_ITEM.QN_PESOITEMPEDIVEND,'
      '       MATERIAL_EMBALAGEM.ID_PRODMATEEMBA,'
      '       MATERIAL_EMBALAGEM.NM_PRODMATEEMBA,'
      '       PEDIDO_VENDA_ITEM.VL_UNITITEMPEDIVEND'
      '  FROM PEDIDO_VENDA_ITEM,'
      '       MATERIAL_EMBALAGEM'
      
        ' WHERE PEDIDO_VENDA_ITEM.ID_MATEEMBA = MATERIAL_EMBALAGEM.ID_MAT' +
        'EEMBA'
      '   AND PEDIDO_VENDA_ITEM.ID_PEDIVEND = :Id_PediVend             '
      '                        ')
    MasterSource = DSUltVend
    MasterFields = 'ID_PEDIVEND'
    DetailFields = 'ID_PEDIVEND'
    Left = 336
    Top = 24
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'Id_PediVend'
        Value = nil
      end>
    object QRItemVendQN_EMBAITEMPEDIVEND: TFloatField
      FieldName = 'QN_EMBAITEMPEDIVEND'
      Required = True
    end
    object QRItemVendQN_PESOITEMPEDIVEND: TFloatField
      FieldName = 'QN_PESOITEMPEDIVEND'
      Required = True
    end
    object QRItemVendID_PRODMATEEMBA: TStringField
      FieldName = 'ID_PRODMATEEMBA'
      Size = 3
    end
    object QRItemVendNM_PRODMATEEMBA: TStringField
      FieldName = 'NM_PRODMATEEMBA'
      Size = 40
    end
    object QRItemVendVL_UNITITEMPEDIVEND: TFloatField
      FieldName = 'VL_UNITITEMPEDIVEND'
      Required = True
      currency = True
    end
  end
  object DSItem: TOraDataSource
    DataSet = QRItemVend
    Left = 368
    Top = 24
  end
end
