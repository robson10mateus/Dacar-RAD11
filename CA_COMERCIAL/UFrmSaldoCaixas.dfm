object FrmSaldoCaixas: TFrmSaldoCaixas
  Left = 468
  Top = 252
  Width = 1226
  Height = 701
  VertScrollBar.Range = 44
  ActiveControl = PageControl1
  Caption = 'Comercial Dacar - Relat'#243'rio de Saldo de Caixas'
  Color = clWindow
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clBlack
  Font.Height = 11
  Font.Name = 'helvetica'
  Font.Pitch = fpVariable
  Font.Style = []
  FormStyle = fsMDIChild
  Icon.Data = {
    
    0000000000000000000000000000}
  Position = poDefault
  Visible = True
  WindowState = wsMaximized
  OnClose = FormClose
  OnShow = FormShow
  TextHeight = 10
  object PageControl1: TPageControl
    Left = 0
    Top = 45
    Width = 1210
    Height = 617
    ActivePage = Tab_Criterio
    Align = alClient
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlack
    Font.Height = 15
    Font.Name = 'Arial'
    Font.Pitch = fpVariable
    Font.Style = []
    ParentFont = False
    TabOrder = 0
    object Tab_Criterio: TTabSheet
      Caption = 'Crit'#233'rio'
      object Label6: TLabel
        Left = 285
        Top = 27
        Width = 56
        Height = 15
        Caption = 'Condi'#231#227'o:'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = 15
        Font.Name = 'Arial'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
      end
      object Label7: TLabel
        Left = 285
        Top = 67
        Width = 62
        Height = 15
        Caption = 'Express'#227'o:'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = 15
        Font.Name = 'Arial'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
      end
      object LCampos: TListBox
        Left = 7
        Top = 10
        Width = 266
        Height = 369
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = 15
        Font.Name = 'Arial'
        Font.Pitch = fpVariable
        Font.Style = []
        ItemHeight = 15
        ParentFont = False
        TabOrder = 0
        OnClick = LCamposClick
      end
      object CBCond: TComboBox
        Left = 365
        Top = 24
        Width = 138
        Height = 23
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = 15
        Font.Name = 'Arial'
        Font.Pitch = fpVariable
        Font.Style = []
        ItemIndex = 0
        ParentFont = False
        TabOrder = 1
        Text = 'Igual a'
        Items.Strings = (
          'Igual a'
          'Diferente de'
          'Maior que'
          'Maior ou igual a'
          'Menor que'
          'Menor ou igual a')
      end
      object RGeou: TRadioGroup
        Left = 548
        Top = 24
        Width = 116
        Height = 25
        Columns = 2
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = 15
        Font.Name = 'Arial'
        Font.Pitch = fpVariable
        Font.Style = [fsBold]
        ItemIndex = 0
        Items.Strings = (
          'E'
          'OU')
        ParentFont = False
        TabOrder = 2
      end
      object EdExpre: TEdit
        Left = 365
        Top = 65
        Width = 299
        Height = 23
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = 15
        Font.Name = 'Arial'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 3
        OnChange = EdExpreChange
      end
      object MResul: TMemo
        Left = 285
        Top = 110
        Width = 468
        Height = 272
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = 15
        Font.Name = 'Arial'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 4
      end
      object BtIncluir: TBitBtn
        Left = 680
        Top = 17
        Width = 75
        Height = 23
        Caption = 'Incluir'
        Default = True
        Enabled = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = 16
        Font.Name = 'Arial'
        Font.Pitch = fpVariable
        Font.Style = []
        Glyph.Data = {
          
          0000}
        ParentFont = False
        TabOrder = 6
        OnClick = BtIncluirClick
      end
      object BtLimpar: TBitBtn
        Left = 680
        Top = 46
        Width = 75
        Height = 23
        Caption = 'Limpar'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = 16
        Font.Name = 'Arial'
        Font.Pitch = fpVariable
        Font.Style = []
        Glyph.Data = {
          
          0000FFFFF70007FFFFFFF0000000}
        ParentFont = False
        TabOrder = 8
        OnClick = BtLimparClick
      end
      object BtAtivar: TBitBtn
        Left = 680
        Top = 75
        Width = 75
        Height = 22
        Caption = 'Ativar'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = 16
        Font.Name = 'Arial'
        Font.Pitch = fpVariable
        Font.Style = []
        Glyph.Data = {
          
          FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
        ParentFont = False
        TabOrder = 5
        OnClick = BtAtivarClick
      end
      object BPESQ: TBitBtn
        Left = 640
        Top = 66
        Width = 23
        Height = 20
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = 16
        Font.Name = 'Arial'
        Font.Pitch = fpVariable
        Font.Style = []
        Glyph.Data = {
          
          33333333333333333333}
        ParentFont = False
        TabOrder = 7
        Visible = False
        OnClick = BPESQClick
      end
    end
    object Tab_Lista: TTabSheet
      Caption = 'Lista'
      ImageIndex = 1
      object DBGrid1: TDBGrid
        Left = 0
        Top = 0
        Width = 1202
        Height = 590
        Hint = 'Atualiza Dados'
        Align = alClient
        DataSource = Ds
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = 15
        Font.Name = 'Arial'
        Font.Pitch = fpVariable
        Font.Style = []
        Options = [dgEditing, dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgConfirmDelete, dgCancelOnExit, dgMultiSelect]
        ParentFont = False
        TabOrder = 0
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clBlack
        TitleFont.Height = 16
        TitleFont.Name = 'Arial'
        TitleFont.Pitch = fpVariable
        TitleFont.Style = [fsBold]
        OnDrawColumnCell = DBGrid1DrawColumnCell
        OnKeyDown = DBGrid1KeyDown
        OnTitleClick = DBGrid1TitleClick
        Columns = <
          item
            Expanded = False
            FieldName = 'ID_CLIENTE'
            Title.Caption = 'C'#243'd'
            Width = 50
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'NM_CLIENTE'
            Title.Caption = 'Raz'#227'o Social'
            Width = 350
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'SALDO'
            Title.Alignment = taRightJustify
            Title.Caption = 'Saldo Caixa'
            Width = 90
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'ID_VALE'
            Title.Caption = 'Ultimo Vale Cx'
            Width = 110
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'NM_CIDADE'
            Title.Caption = 'Nome Cidade'
            Width = 250
            Visible = True
          end>
      end
    end
  end
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 1210
    Height = 45
    Align = alTop
    Color = clWindow
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlack
    Font.Height = 15
    Font.Name = 'Arial'
    Font.Pitch = fpVariable
    Font.Style = []
    ParentBackground = False
    ParentFont = False
    TabOrder = 1
    object SB_ATUAL: TSpeedButton
      Left = 5
      Top = 5
      Width = 35
      Height = 35
      Hint = 'Atualizar Dados Listados'
      Flat = True
      Glyph.Data = {
        
        F0F0F0FFFFFFFFFFFFFFFFFFFCFCFC000000}
      NumGlyphs = 2
      ParentShowHint = False
      ShowHint = True
      OnClick = SB_ATUALClick
    end
    object SB_Relatorio: TSpeedButton
      Left = 40
      Top = 5
      Width = 35
      Height = 35
      Hint = 'Imprimir Dados Listados'
      Flat = True
      Glyph.Data = {
        
        5757575959595959594F4F4F777777FAFAFA}
      NumGlyphs = 2
      ParentShowHint = False
      ShowHint = True
      OnClick = SB_RelatorioClick
    end
    object Sb_Sair: TSpeedButton
      Left = 75
      Top = 5
      Width = 35
      Height = 35
      Hint = 'Sair'
      Flat = True
      Glyph.Data = {
        
        FEFEFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF00}
      ParentShowHint = False
      ShowHint = True
      OnClick = Sb_SairClick
    end
  end
  object Qr: TOraQuery
    SQLDelete.Strings = (
      'DELETE FROM INTEGRADO_ALOJAMENTO'
      'WHERE'
      '  ID_LOTE = :ID_LOTE')
    SQLUpdate.Strings = (
      'UPDATE INTEGRADO_ALOJAMENTO'
      'SET'
      ' PROX_ENTREGA_RACAO = :PROX_ENTREGA_RACAO'
      '       '
      'WHERE'
      '  ID_LOTE = :OLD_ID_LOTE')
    SQLLock.Strings = (
      'SELECT * FROM INTEGRADO_ALOJAMENTO'
      'WHERE'
      '  ID_LOTE = :ID_LOTE'
      'FOR UPDATE NOWAIT')
    SQLRefresh.Strings = (
      'WHERE'
      '  ID_LOTE = :ID_LOTE')
    Session = FrmPrincipal.DB
    SQL.Strings = (
      'SELECT distinct C.ID_CLIENTE,'
      '       C.NM_CLIENTE,'
      '       C.FL_PESSCLIE,'
      '       C.FL_ATIVCLIE,'
      '       M.ID_CIDADE,'
      '       M.NM_CIDADE,'
      '       VEN.ID_VENDEDOR,'
      '       VEN.NM_VENDEDOR,'
      '       SAL.ID_VALE,'
      '       NVL(SAL.SALDO,0) SALDO,'
      '       CG.ID_GRUPCLIE,'
      '       CG.NM_GRUPCLIE'
      ''
      ''
      '  FROM CLIENTE C'
      ' Inner Join BAIRRO B'
      '    On C.ID_BAIRRO = B.ID_BAIRRO'
      ' Inner Join Cidade M'
      '    On B.ID_CIDADE = M.ID_CIDADE'
      ' Left Join VALE_CAIXA VC'
      '    On VC.ID_CLIENTE = C.ID_CLIENTE '
      'LEFT JOIN ( SELECT V.ID_CLIENTE,'
      '                   V.ID_VALE,'
      
        '                   NVL( V.SALDO_ANTERIOR,0)  + NVL( V.QTD_ENTREG' +
        'UE,0) -  NVL(V.QTD_DEVOLVIDA,0) SALDO'
      '             FROM VALE_CAIXA V'
      
        '            INNER JOIN ( Select ID_CLIENTE , MAX(ID_VALE) ID_VAL' +
        'E'
      '                           From VALE_CAIXA'
      '                          Where FL_STATUS <> '#39'CN'#39
      '                          GROUP BY ID_CLIENTE) UV'
      '               On V.ID_CLIENTE = UV.ID_CLIENTE'
      '              AND V.ID_VALE   =  UV.ID_VALE ) SAL'
      '   ON SAL.ID_CLIENTE = C.ID_CLIENTE'
      '  Left Join Cliente_Vendedor CV'
      '  /*On CV.ID_CLIENTE = C.ID_CLIENTE'
      
        '  FABRICIO 25/01/2018 : ADD CONDI'#199#195'O DE FLAG DE VENDENDOR PADR'#195'O' +
        '. '
      
        '  HA CASOS EM QUE O MESMO CLIENTE POSSUI N VENDEDORES VINCULADOS' +
        ', O QUE '#39'DUPLICA'#39' LISTAGEM. '
      '  COM A CONDI'#199'AO, LISTA SOMENTE REFERENTE AO VENDEDOR PADRAO.*/'
      '  On (CV.ID_CLIENTE = C.ID_CLIENTE AND CV.FL_PADRVENDCLIE = '#39'S'#39')'
      '  Left Join logistica_regiao LR'
      '    On C.ID_REGILOGI = LR.ID_REGILOGI'
      '  Left Join logistica_subregiao LS'
      '    On C.ID_SUBRLOGI = LS.ID_SUBRLOGI'
      '  Inner Join VENDEDOR VEN ON (VEN.ID_VENDEDOR = CV.ID_VENDEDOR)'
      '  Left Join CLIENTE_GRUPO CG ON CG.ID_GRUPCLIE = C.ID_GRUPCLIE'
      '&Macro'
      ''
      'ORDER BY VEN.ID_VENDEDOR, VEN.NM_VENDEDOR, C.NM_CLIENTE')
    FetchAll = True
    CachedUpdates = True
    Left = 124
    Top = 9
    MacroData = <
      item
        Name = 'Macro'
      end>
    object QrID_CLIENTE: TFloatField
      FieldName = 'ID_CLIENTE'
      Required = True
    end
    object QrNM_CLIENTE: TStringField
      FieldName = 'NM_CLIENTE'
      Required = True
      Size = 40
    end
    object QrID_CIDADE: TFloatField
      FieldName = 'ID_CIDADE'
      Required = True
    end
    object QrNM_CIDADE: TStringField
      FieldName = 'NM_CIDADE'
      Required = True
      Size = 40
    end
    object QrID_VALE: TFloatField
      FieldName = 'ID_VALE'
    end
    object QrSALDO: TFloatField
      FieldName = 'SALDO'
    end
    object QrFL_PESSCLIE: TStringField
      FieldName = 'FL_PESSCLIE'
      Required = True
      Size = 1
    end
    object QrFL_ATIVCLIE: TStringField
      FieldName = 'FL_ATIVCLIE'
      Required = True
      Size = 1
    end
    object QrID_VENDEDOR: TFloatField
      FieldName = 'ID_VENDEDOR'
      Required = True
    end
    object QrNM_VENDEDOR: TStringField
      FieldName = 'NM_VENDEDOR'
      Required = True
      Size = 40
    end
    object QrID_GRUPCLIE: TFloatField
      FieldName = 'ID_GRUPCLIE'
    end
    object QrNM_GRUPCLIE: TStringField
      FieldName = 'NM_GRUPCLIE'
      Size = 40
    end
  end
  object Ds: TOraDataSource
    DataSet = Qr
    Left = 156
    Top = 10
  end
end
