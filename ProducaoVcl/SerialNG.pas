...
var
  ...
  S : String;
begin
  {stringlist to hold key or value names during QSearch}
...
type TMaxSize = array[0..MaxLongInt-1] of AnsiChar;
     PMaxSize = ^TMaxSize;
begin
  if Dest <> Nil then
    begin
...
begin
  ...
  MyDCB.ErrorChar := AnsiChar(fParityErrorChar);
  MyDCB.EvtChar := AnsiChar(fEventChar);
  MyDCB.StopBits := fStopBits;
  MyDCB.ByteSize := fDataBits;
  MyDCB.XONChar := AnsiChar(fXONChar);
  MyDCB.XOFFChar := AnsiChar(fXOFFChar);
